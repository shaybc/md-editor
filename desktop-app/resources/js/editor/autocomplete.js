(function(window, document) {
  ...
  function registerMarkdownViewerAutocomplete(app, deps) {
    ...
    function hideLinkAutocomplete() {
      ...
      getMarkdownEditor()?.removeAttribute("aria-activedescendant");
    }
    ...
    function renderLinkAutocomplete() {
      ...
      if (context.type !== "tag" && context.type !== "frontmatter-tag" && !deps.getIsFolderOpen()) {
        ...
        getMarkdownEditor()?.setAttribute("aria-activedescendant", `link-autocomplete-option-${selectedIndex}`);
      }
    ...
    function acceptLinkAutocomplete(index = linkAutocompleteState?.selectedIndex || 0) {
      ...
      if (activeEditorCommands?.replaceActiveEditorRange) {
        activeEditorCommands.replaceActiveEditorRange(state.replaceStart, state.replaceEnd + closingSyntaxLength, insertText);
        activeEditorCommands.setActiveEditorSelection?.(nextPosition, nextPosition);
      } else {
        markdownEditor.value = value.slice(0, state.replaceStart) + insertText + value.slice(state.replaceEnd + closingSyntaxLength);
        markdownEditor.selectionStart = markdownEditor.selectionEnd = nextPosition;
        markdownEditor.dispatchEvent(new Event("input"));
      }
      hideLinkAutocomplete();
      activeEditorCommands?.focusActiveEditor?.() || markdownEditor.focus();
      return true;
    }
