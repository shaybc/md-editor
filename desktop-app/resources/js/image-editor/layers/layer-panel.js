...
(function(global) {
  ...
  class ImageEditorLayerPanel {
    ...
    openContextMenu(event) {
      ...
      const catalogLabel = (label) => String(label || "").replace(/<br\s*\/?\s*>/gi, " ").replace(/&amp;/g, "&");
      const catalogs = namespace.ImageEditorAdjustmentCatalogs || { tune: [], layerEffects: [] };
      const tuneMenuItems = catalogs.tune.map(([action, type, label]) => ({
        id: action,
        label: catalogLabel(label),
        effectIcon: type,
        disabled: !canStyle
      }));
      const effectMenuItems = catalogs.layerEffects.map(([action, type, label]) => ({
        id: action,
        label: catalogLabel(label) + "…",
        effectIcon: type,
        disabled: !canStyle
      }));
      const effectRemoveMenuItems = catalogs.layerEffects.map(([action, type, label]) => ({
        id: action.replace(/^edit-/, "remove-"),
        label: catalogLabel(label),
        effectIcon: type,
        disabled: !canStyle || !styleLayers.some((layer) => (layer.effects || []).some((effect) => effect?.type === type))
      })).filter((item) => !item.disabled);
      const styleRemoveMenuItems = [
      ...
      this.contextMenu.show(event.clientX, event.clientY, [
        ...
        { label: "Style", effectIcon: "effects", disabled: !canStyle, children: [
          ...
          ...(styleRemoveMenuItems.length ? [
            { separator: true },
            { label: "Remove", icon: "bi-x-square", children: styleRemoveMenuItems }
          ] : [])
        ] },
        { label: "Tune", icon: "bi-sliders", disabled: !canStyle, children: [
          ...tuneMenuItems,
          { separator: true },
          { id: "apply-grayscale", label: "Grayscale", effectIcon: "grayscale", disabled: !canStyle },
          ...(hasGrayscale ? [
            { separator: true },
            { label: "Remove", icon: "bi-x-square", children: [
              { id: "remove-grayscale", label: "Grayscale", effectIcon: "grayscale" }
            ] }
          ] : [])
        ] },
        { label: "Effects", effectIcon: "effects", disabled: !canStyle, children: [
          ...effectMenuItems,
          ...(effectRemoveMenuItems.length ? [
            { separator: true },
            { label: "Remove", icon: "bi-x-square", children: effectRemoveMenuItems }
          ] : [])
        ] },
        { separator: true },
