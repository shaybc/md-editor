// Normalized non-destructive Monochromatic descriptors for image-editor layers.
(function(global) {
  "use strict";

  const namespace = global.MarkdownViewerImageEditor = global.MarkdownViewerImageEditor || {};
  const TYPE = "monochromatic";
  ...
  function normalizeColor(value) {
    ...
    return /^#[0-9a-f]{6}$/i.test(color) ? color.toUpperCase() : "#735437";
  }

  /** Create a complete Monochromatic descriptor from optional persisted values. */
  ...
  /** Return the normalized Monochromatic effect currently attached to a layer. */
  function get(layer) {
    const effect = (layer?.effects || []).find((entry) => entry?.type === TYPE);
    return effect ? normalize(effect) : null;
  }

  /** Add or replace a layer's single Monochromatic effect. */
  ...
  /** Remove Monochromatic without changing any other layer effects. */
  ...
  /** Normalize persisted Monochromatic descriptors throughout a document. */
  ...
  namespace.ImageEditorMonochromaticEffect = Object.freeze({ TYPE, normalize, get, upsert, remove, normalizeDocument });
