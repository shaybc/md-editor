// Normalized non-destructive drop-shadow descriptors for image-editor layers.
(function(global) {
  "use strict";

  const namespace = global.MarkdownViewerImageEditor = global.MarkdownViewerImageEditor || {};
  const TYPE = "drop-shadow";
  ...
  /** Create a complete drop-shadow descriptor from optional persisted values. */
  function normalize(effect = {}) {
    return {
      ...
      distance: clamp(effect.distance, 0, 1000, 10),
      spread: clamp(effect.spread, 0, 1, 0),
      blur: clamp(effect.blur, 0, 1000, 12)
    };
  }

  /** Return the normalized drop shadow currently attached to a layer. */
  function get(layer) {
    const effect = (layer?.effects || []).find((entry) => entry?.type === TYPE);
    return effect ? normalize(effect) : null;
  }

  /** Add or replace a layer's single drop-shadow effect. */
  ...
  /** Remove the drop-shadow effect without changing other layer effects. */
  ...
  /** Normalize persisted drop-shadow descriptors throughout a document. */
  ...
  namespace.ImageEditorDropShadowEffect = Object.freeze({ TYPE, normalize, get, upsert, remove, normalizeDocument });
