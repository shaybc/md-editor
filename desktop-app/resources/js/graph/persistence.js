(function(global) {
  global.registerMarkdownViewerGraphPersistence = function registerMarkdownViewerGraphPersistence(app, deps) {
    ...
    const GRAPH_SNAPSHOT_CACHE_FILE_LIMIT = 5000;
  ...
  function normalizeGraphViewConfig(config) {
    ...
    return {
      ...
      showExternalJars: mergedSource.showExternalJars !== false,
      showMissingDependencies: mergedSource.showMissingDependencies !== false,
  ...
  function isGraphFileNode(node) {
    if (!node || typeof node !== "object") return false;
    const type = String(node.type || "file").trim() || "file";
    return type === "file";
  }

  function getGraphNodePath(node) {
    if (!node || typeof node !== "object") return "";
    return String(node.path || node.fullPath || node.file?.webkitRelativePath || node.file?.name || node.name || "").replace(/\\/g, "/");
  }

  function getGraphNodeName(node) {
    const path = getGraphNodePath(node);
    return String(node.name || getFileName(path || node?.label || node?.id || "document.md") || "document.md");
  }

  function createGraphFileDataFromNode(node) {
    if (!isGraphFileNode(node)) return null;
    const path = getGraphNodePath(node);
    const id = node.id || normalizeGraphNodeName(path || node.name || "");
    if (!id) return null;
    const fileData = {
      id,
      path,
      name: getGraphNodeName({ ...node, path }),
      tags: normalizeFileTagList(node.tags || [])
    };
    if (node.fullPath && node.fullPath !== path) fileData.fullPath = node.fullPath;
    if (node.content !== undefined) fileData.content = node.content;
    if (node.handle) fileData.handle = node.handle;
    if (node.file) fileData.file = node.file;
    if (node.originalSourcePath) fileData.originalSourcePath = node.originalSourcePath;
    if (node.originalSourceName) fileData.originalSourceName = node.originalSourceName;
    if (node.status && node.status !== "current") fileData.status = node.status;
    return fileData;
  }

  function getGraphSnapshotFileEntries(snapshot) {
    ...
    return (Array.isArray(snapshot.nodes) ? snapshot.nodes : [])
      .map(createGraphFileDataFromNode)
      .filter(Boolean);
  }
  ...
  function normalizeGraphSnapshot(snapshot) {
    ...
    if (!Array.isArray(normalizedSnapshot.files) || !normalizedSnapshot.files.length) {
      const derivedFiles = getGraphSnapshotFileEntries(normalizedSnapshot);
      if (derivedFiles.length) normalizedSnapshot.files = derivedFiles;
    }
  ...
  function getGraphSnapshotFilesForComparison(snapshot) {
    if (!snapshot || typeof snapshot !== "object") return [];
    const files = getGraphSnapshotFileEntries(snapshot);
  ...
  function hideGraphBanner() {
    window.clearTimeout(graphUpdateBannerTimeout);
    graphUpdateBannerTimeout = null;
    const banner = document.getElementById("graph-update-banner");
  ...
  function refreshGraphModeNoticesForTab(tab) {
    const graphTab = tab && tab.type === "graph" ? tab : null;
    updateSavedGraphModePill(graphTab);
    if (!graphTab) {
      hideGraphStaleModal();
      closeGraphComparisonDetailsModal();
      hideGraphBanner();
      activeGraphComparisonDetailsModel = null;
      return;
    }
  ...
  function clearGraphPersistenceCaches() {
    graphParsedFileCache.clear();
    graphSnapshotCache.clear();
  }

  function isScopedSavedGraphView(tab) {
    const config = tab?.graphViewConfig || tab?.graphDocument?.viewConfig || {};
    const mode = String(config.mode || "").trim();
    return (
      ["local", "full-local", "full-network", "cluster", "custom"].includes(mode)
      || !!config.focusNodeId
      || (Array.isArray(config.clusterNodeIds) && config.clusterNodeIds.length > 0)
      || (Array.isArray(config.allowedNodeIds) && config.allowedNodeIds.length > 0)
    );
  }

  function normalizeGraphScopePathForComparison(value) {
    return String(value || "")
      .replace(/\\/g, "/")
      .replace(/\/+/g, "/")
      .replace(/\/+$|^\s+|\s+$/g, "")
  ...
  function getCurrentRootGraphScopeKeyForComparison() {
    const normalizedPath = normalizeGraphScopePathForComparison(activeFolderPath || activeFolderName || "Graph View");
    return normalizedPath ? `root-folder:${normalizedPath}` : "";
  }

  function shouldCompareSavedGraphWithCurrentFolder(tab, options = {}) {
    if (!tab?.graphSnapshot || !folderMarkdownFiles.length) return false;
    if (isScopedSavedGraphView(tab)) return false;

    const rootScopeKey = getCurrentRootGraphScopeKeyForComparison();
    if (tab.graphScopeKey) return !!rootScopeKey && tab.graphScopeKey === rootScopeKey;

    const savedFolderName = String(tab.graphSnapshot?.folderName || tab.folderName || tab.title || "").trim().toLowerCase();
    const currentFolderName = String(activeFolderName || "").trim().toLowerCase();
    if (savedFolderName && currentFolderName && savedFolderName === currentFolderName) return true;

    return options.force === true && options.allowUnscopedComparison === true;
  }

  async function promptSavedGraphTabForCurrentFolder(tab) {
    if (!tab || tab.type !== "graph" || !isFileBackedGraphTab(tab)) return false;
    if (!shouldCompareSavedGraphWithCurrentFolder(tab, { force: true })) return false;
    const graphDocumentType = tab.graphDocument?.documentType || inferLegacyGraphDocumentType(tab.graphSnapshot);
  ...
  function getGraphEndpointId(endpoint) {
    if (endpoint && typeof endpoint === "object") return endpoint.id || endpoint.path || endpoint.name || "";
    return endpoint;
  }

  function stripGraphNodeForView(node) {
    const strippedNode = cloneGraphPersistenceValue(node || {});
    if (!strippedNode || typeof strippedNode !== "object") return strippedNode;
    [
      "content",
      "markdown",
      "body",
      ...
      delete strippedNode[key];
    });
    if (strippedNode.status === "current") delete strippedNode.status;
    return strippedNode;
  }

  function stripGraphLinkForView(link) {
    const strippedLink = cloneGraphPersistenceValue(link || {});
    if (!strippedLink || typeof strippedLink !== "object") return strippedLink;
    strippedLink.source = getGraphEndpointId(strippedLink.source);
    strippedLink.target = getGraphEndpointId(strippedLink.target);
    delete strippedLink.index;
    if (strippedLink.status === "current") delete strippedLink.status;
    return strippedLink;
  }

  function createGraphFileMap(snapshot) {
    const files = Array.isArray(snapshot?.files) ? snapshot.files : [];
    return new Map(files.map((file) => [file?.id, file]).filter(([id]) => !!id));
  }

  function compactGraphFileNodeForView(node, fileData) {
    const source = {
      ...(fileData && typeof fileData === "object" ? fileData : {}),
      ...(node && typeof node === "object" ? node : {})
    };
    const path = String(source.path || source.fullPath || "").replace(/\\/g, "/");
    const compactNode = {
      id: source.id || normalizeGraphNodeName(path || source.name || ""),
      type: "file"
    };
    if (path) compactNode.path = path;
    const tags = normalizeFileTagList(source.tags || []);
    compactNode.tags = tags;
    if (source.status && source.status !== "current") compactNode.status = source.status;
    return compactNode;
  }
  ...
  function stripGraphSnapshotContent(snapshot, options = {}) {
    ...
    const omitFiles = options.omitFiles === true;
    const fileMap = createGraphFileMap(strippedSnapshot);

    strippedSnapshot.nodes = Array.isArray(strippedSnapshot.nodes)
      ? strippedSnapshot.nodes.map((node) => {
          if (isGraphFileNode(node)) return compactGraphFileNodeForView(node, fileMap.get(node?.id));
          return stripGraphNodeForView(node);
        })
      : [];
    strippedSnapshot.links = Array.isArray(strippedSnapshot.links) ? strippedSnapshot.links.map(stripGraphLinkForView) : [];
    if (omitFiles) delete strippedSnapshot.files;
    else {
  ...
  function getPersistableGraphViewLayout(graphLayout) {
    return shouldPersistGraphViewLayout(graphLayout) ? cloneGraphPersistenceValue(graphLayout) : undefined;
  }
  ...
  function serializeGraphViewDocument(tab) {
    ...
    const viewDocument = {
      ...
      snapshot: stripGraphSnapshotContent(graphDocument.snapshot, { omitFiles: true })
    };
    delete viewDocument.graphLayout;
    const graphLayout = getPersistableGraphViewLayout(graphDocument.graphLayout);
    if (graphLayout !== undefined) viewDocument.graphLayout = graphLayout;
    const normalizedDocument = normalizeGraphDocument(viewDocument);
    normalizedDocument.snapshot = stripGraphSnapshotContent(normalizedDocument.snapshot, { omitFiles: true });
  ...
  function normalizeGraphDocument(document) {
    ...
    if (source.graphScopeKey) {
      normalized.graphScopeKey = String(source.graphScopeKey || "").trim();
    }
  ...
  function serializeGraphTab(tab, options) {
    ...
    return normalizeGraphDocument({
      ...
      graphScopeKey: tab?.graphScopeKey || existingDocument.graphScopeKey || ""
    });
  }
  ...
  function deserializeGraphDocument(document) {
    ...
    if (normalizedDocument.graphScopeKey) {
      graphData.graphScopeKey = normalizedDocument.graphScopeKey;
    }
  ...
  function isGraphPerfLoggingEnabled() {
    return (
      typeof appDebugLog === "function"
      || global.MD_VIEWER_PERF === true
      || global.localStorage?.getItem("MD_VIEWER_PERF") === "1"
    );
  }
  ...
  function logGraphPerf(label, startTime, details = {}) {
    ...
    const payload = { durationMs: duration, ...details };
    console.info(`[Perf] ${label}: ${duration}ms`, details);
    if (typeof appDebugLog === "function") {
      void appDebugLog("info", `[graph-perf] ${label}`, payload);
    }
  }
  ...
  function createGraphPerfSession(label, details = {}) {
    ...
    return {
      ...
      mark(name, markDetails = {}) {
        ...
        marks.push(mark);
        if (typeof appDebugLog === "function") {
          void appDebugLog("info", `[graph-perf] ${label} :: ${name}`, { ...details, ...mark });
        }
        lastMarkTime = now;
      },
      end(endDetails = {}) {
        ...
        if (typeof appDebugLog === "function") {
          void appDebugLog("info", `[graph-perf] ${label} complete`, { ...summary, marks });
        }
  ...
  function nowForGraphStoragePerf() {
    return typeof performance !== "undefined" ? performance.now() : 0;
  }

  function elapsedGraphStoragePerfMs(startTime) {
  ...
  function shouldUseGraphSnapshotCache(files, options = {}) {
    if (options.skipSnapshotCache) return false;
    return (files || []).length <= GRAPH_SNAPSHOT_CACHE_FILE_LIMIT;
  }
  ...
  async function createGraphSnapshot(files, folderName, options = {}) {
    ...
    const sourceFiles = files || [];
    const useSnapshotCache = shouldUseGraphSnapshotCache(sourceFiles, options);
    const cacheKeyStart = typeof performance !== "undefined" ? performance.now() : 0;
    const snapshotCacheKey = useSnapshotCache ? getGraphSnapshotCacheKey(sourceFiles, folderName) : "";
    logGraphPerf("graph snapshot cache key", cacheKeyStart, {
      files: sourceFiles.length,
      cacheKeyLength: snapshotCacheKey.length,
      skipped: !useSnapshotCache,
      cacheFileLimit: GRAPH_SNAPSHOT_CACHE_FILE_LIMIT
    });
    ...
    snapshotFiles = await mapGraphFilesWithConcurrency(sourceFiles, async (fileEntry) => {
      ...
      if (!parsedFile) {
        ...
        parsedFile = {
          ...
          sourceFile: typeof extractSourceFileFromFrontmatter === "function" ? extractSourceFileFromFrontmatter(fileContent) : "",
          unresolvedDependencies: typeof extractUnresolvedDependencies === "function" ? extractUnresolvedDependencies(fileContent) : []
        };
      ...
      return {
        ...
        originalSourcePath: parsedFile.sourceFile || null,
        originalSourceName: parsedFile.sourceFile ? getFileName(parsedFile.sourceFile) : null,
        status: "current",
        tags: (parsedFile.tags || []).slice(),
        markdownLinks: (parsedFile.markdownLinks || []).slice(),
        unresolvedDependencies: (parsedFile.unresolvedDependencies || []).slice()
      };
    }, options);
    ...
    const missingDependencyStart = typeof performance !== "undefined" ? performance.now() : 0;
    const missingDependencyIndex = new Map();
        const missingDependencyId = (dependency) => `missing:${String(dependency?.language || "unknown").trim().toLowerCase()}:${String(dependency?.symbol || "").trim().toLowerCase()}`;
    const missingDependencyLabel = (symbol) => {
      const normalized = String(symbol || "").replace(/\.\*$/, "");
      return normalized.split(".").filter(Boolean).pop() || normalized || "Missing dependency";
    };
    for (let index = 0; index < snapshotFiles.length; index += 1) {
      if (index > 0 && index % 1000 === 0) await new Promise((resolve) => setTimeout(resolve, 0));
      const snapshotFile = snapshotFiles[index];
      const source = snapshotFile.id;
      (snapshotFile.unresolvedDependencies || []).forEach((dependency) => {
        const symbol = String(dependency?.symbol || "").trim();
        if (!symbol) return;
        const target = missingDependencyId(dependency);
        if (!missingDependencyIndex.has(target)) {
          missingDependencyIndex.set(target, dependency);
          nodes.push({
            id: target,
            label: missingDependencyLabel(symbol),
            type: "missing-dependency",
            status: "unresolved",
            qualifiedName: symbol,
            language: dependency.language || "unknown",
            missingKind: dependency.kind || "class",
            wildcard: dependency.wildcard === true,
            staticImport: dependency.staticImport === true
          });
        }
        const edgeKey = `${source}->${target}:missing-dependency`;
        if (seenEdges.has(edgeKey)) return;
        seenEdges.add(edgeKey);
        links.push({ source, target, type: "missing-dependency", status: "unresolved" });
      });
    }
    logGraphPerf("graph snapshot missing dependency build", missingDependencyStart, { missing: missingDependencyIndex.size, links: links.length });
  ...
  function hideInactiveGraphRenders(activeGraphTabId) {
    if (graphViewCanvas) {
      graphViewCanvas.querySelectorAll(".graph-tab-render").forEach((wrapper) => {
        wrapper.classList.toggle("hidden", wrapper.dataset.graphTabId !== String(activeGraphTabId || ""));
      });
    }
    ...
    document.querySelectorAll(".graph-quick-action").forEach((node) => {
      node.classList.toggle("hidden", node.dataset.graphTabId !== String(activeGraphTabId || ""));
    });
  }
  ...
  function loadTabsFromStorage() {
    try {
      const tabsFromStorage = JSON.parse(localStorage.getItem(STORAGE_KEY)) || null;
      const payload = deps.tabSessionPersistence?.isSessionPayload?.(tabsFromStorage)
        ? tabsFromStorage
        : null;
      if (typeof appDebugLog === "function") {
        void appDebugLog("debug", "[tabs-session] Loaded tabs from localStorage", {
          version: payload?.version || null,
          count: Array.isArray(payload?.tabs) ? payload.tabs.length : 0,
          titles: Array.isArray(payload?.tabs) ? payload.tabs.map((tab) => tab?.title).filter(Boolean) : []
        });
      }
      return payload || { version: 2, activeTabId: null, tabs: [] };
    } catch (e) {
      if (typeof appDebugLog === "function") {
        void appDebugLog("warning", "[tabs-session] Failed to load tabs from localStorage", e);
      }
  ...
  function getPersistableTab(tab) {
    if (tab?.type === "large-file") {
      ...
      persistedTab.content = "";
      persistedTab.savedContent = "";
      if (persistedTab.largeFileSource) {
        persistedTab.largeFileSource = {
          ...persistedTab.largeFileSource,
          file: null,
          content: undefined
        };
  ...
  function getCompactPersistableTab(tab) {
    if (tab?.type === "large-file") return getPersistableTab(tab);
    if (!tab || tab.type !== "graph") return tab;
  ...
  function saveTabsToStorage(tabsArr) {
    const perf = typeof createGraphPerfSession === "function"
      ? createGraphPerfSession("tabs session save", {
        tabCount: (tabsArr || []).length,
        activeTabId: activeTabId || null,
        graphTabs: (tabsArr || []).filter((tab) => tab?.type === "graph").length
      })
      : null;
    const totalStart = nowForGraphStoragePerf();
    try {
      let stepStart = nowForGraphStoragePerf();
      (tabsArr || []).forEach((tab) => syncGraphTabDocument(tab));
      perf?.mark("sync graph tab documents", {
        durationMs: elapsedGraphStoragePerfMs(stepStart)
      });

      stepStart = nowForGraphStoragePerf();
      const includeInlineDraft = !(typeof isNeutralinoRuntime === "function" && isNeutralinoRuntime());
      const persistablePayload = deps.tabSessionPersistence?.createBrowserPayload
        ? deps.tabSessionPersistence.createBrowserPayload(tabsArr || [], activeTabId || null, { includeInlineDraft })
        : { version: 2, updatedAt: Date.now(), activeTabId: activeTabId || null, tabs: [] };
      perf?.mark("create browser session payload", {
        durationMs: elapsedGraphStoragePerfMs(stepStart),
        includeInlineDraft,
        payloadTabs: Array.isArray(persistablePayload.tabs) ? persistablePayload.tabs.length : 0,
        dirtyGraphTabs: (persistablePayload.tabs || []).filter((tab) => tab?.type === "graph" && tab.dirty).length,
        graphDrafts: (persistablePayload.tabs || []).filter((tab) => tab?.type === "graph" && tab.draft).length
      });

      stepStart = nowForGraphStoragePerf();
      const serializedPayload = JSON.stringify(persistablePayload);
      perf?.mark("stringify browser session payload", {
        durationMs: elapsedGraphStoragePerfMs(stepStart),
        bytesApprox: serializedPayload.length
      });

      stepStart = nowForGraphStoragePerf();
      localStorage.setItem(STORAGE_KEY, serializedPayload);
      perf?.mark("write browser session localStorage", {
        durationMs: elapsedGraphStoragePerfMs(stepStart),
        bytesApprox: serializedPayload.length
      });
      if (typeof appDebugLog === "function") {
        void appDebugLog("debug", "[tabs-session] Saved tabs to localStorage", {
          version: persistablePayload.version || null,
          count: Array.isArray(persistablePayload.tabs) ? persistablePayload.tabs.length : 0,
          titles: (persistablePayload.tabs || []).map((tab) => tab?.title).filter(Boolean)
        });
      }
      if (typeof scheduleTabsProfileWrite === "function") {
        stepStart = nowForGraphStoragePerf();
        scheduleTabsProfileWrite(tabsArr || []);
        perf?.mark("schedule tabs profile write", {
          durationMs: elapsedGraphStoragePerfMs(stepStart)
        });
      }
      perf?.end({ completed: true, durationMs: elapsedGraphStoragePerfMs(totalStart) });
    } catch (e) {
      perf?.mark("tabs session save failed", {
        durationMs: elapsedGraphStoragePerfMs(totalStart),
        error: e?.name || e?.message || String(e)
      });
      if (e?.name === "QuotaExceededError") {
        try {
          graphTabStorageCompactMode = true;
          const compactPayload = deps.tabSessionPersistence?.createBrowserPayload
            ? deps.tabSessionPersistence.createBrowserPayload(tabsArr || [], activeTabId || null, { includeInlineDraft: false })
            : { version: 2, updatedAt: Date.now(), activeTabId: activeTabId || null, tabs: [] };
          localStorage.setItem(STORAGE_KEY, JSON.stringify(compactPayload));
          if (typeof appDebugLog === "function") {
            void appDebugLog("warning", "[tabs-session] Saved compact tabs after localStorage quota error", {
              count: Array.isArray(compactPayload.tabs) ? compactPayload.tabs.length : 0
            });
          }
          if (typeof scheduleTabsProfileWrite === "function") {
            scheduleTabsProfileWrite(tabsArr || []);
          }
          perf?.end({ completed: true, compactMode: true, durationMs: elapsedGraphStoragePerfMs(totalStart) });
          return;
        } catch (compactError) {
          if (typeof appDebugLog === "function") {
            void appDebugLog("error", "[tabs-session] Failed to save compact tabs to localStorage", compactError);
          }
          console.warn('Failed to save compact tabs to localStorage:', compactError);
        }
      } else {
        if (typeof appDebugLog === "function") {
          void appDebugLog("error", "[tabs-session] Failed to save tabs to localStorage", e);
        }
        console.warn('Failed to save tabs to localStorage:', e);
      }
      perf?.end({ completed: false, durationMs: elapsedGraphStoragePerfMs(totalStart) });
    }
  }
  ...
  function loadActiveTabId() {
    ...
    if (typeof appDebugLog === "function") {
      void appDebugLog("debug", "[tabs-session] Loaded active tab id", { id });
    }
    return id;
  }
  ...
  function saveActiveTabId(id) {
    ...
    if (typeof appDebugLog === "function") {
      void appDebugLog("debug", "[tabs-session] Saved active tab id", { id: id || null });
    }
  }
