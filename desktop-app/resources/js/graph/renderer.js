(function(global) {
  global.registerMarkdownViewerGraphRenderer = function registerMarkdownViewerGraphRenderer(app, deps) {
    ...
    function hasReachedOpenTabLimit() {
      const maxOpenTabs = typeof getMaxOpenTabs === "function" ? getMaxOpenTabs() : 40;
      if (tabs.length < maxOpenTabs) return false;
      alert(`Maximum of ${maxOpenTabs} tabs reached. Please close an existing tab to open a new one.`);
      return true;
    }

    const withPausedGraphDraftPersistence = (reason, work) => {
      if (typeof withPausedTabsSessionProfileWrites === "function") {
        return withPausedTabsSessionProfileWrites(reason, work);
      }
      return work();
    };
    ...
    function getGraphRenderSignatureForTab(tab, graphSnapshot, graphViewConfig) {
      ...
      return JSON.stringify({
        ...
        graphColors: {
          nodeDefault: typeof getGraphNodeDefaultColor === "function" ? getGraphNodeDefaultColor() : null,
          link: typeof getGraphLinkColor === "function" ? getGraphLinkColor() : null,
          externalDependency: typeof getGraphExternalDependencyColor === "function" ? getGraphExternalDependencyColor() : null,
          externalDependencyLine: typeof getGraphExternalDependencyLineColor === "function" ? getGraphExternalDependencyLineColor() : null,
          missingDependency: typeof getGraphMissingDependencyColor === "function" ? getGraphMissingDependencyColor() : null,
          missingDependencyLine: typeof getGraphMissingDependencyLineColor === "function" ? getGraphMissingDependencyLineColor() : null,
          tagNode: typeof getGraphTagNodeColor === "function" ? getGraphTagNodeColor() : null,
          tagLine: typeof getGraphTagLineColor === "function" ? getGraphTagLineColor() : null,
          clusterNode: typeof getGraphClusterNodeColor === "function" ? getGraphClusterNodeColor() : null,
          findHighlight: typeof getGraphFindHighlightColor === "function" ? getGraphFindHighlightColor() : null
        },
        largeMapHoverPreferences: typeof getLargeMapHoverPreferences === "function" ? getLargeMapHoverPreferences() : null
      });
    }

    function canReuseGraphRender(tab) {
      const graphSnapshot = tab?.type === "graph" ? (tab.graphComparisonSnapshot || tab.graphSnapshot || null) : null;
      if (!tab?.id) return false;
      const graphViewConfig = normalizeGraphViewConfig(tab.graphViewConfig);
      const cachedRender = graphRenderCache.get(tab.id);
      const tabIdSelector = typeof CSS !== "undefined" && typeof CSS.escape === "function"
        ? CSS.escape(String(tab.id))
        : String(tab.id).replace(/"/g, '\\"');
      const hasQuickAction = tab.graphViewKind === "health-report"
        || !!cachedRender?.quickActionWrapper
        || !!document.querySelector(`.graph-quick-action[data-graph-tab-id="${tabIdSelector}"]`);
      return !!(
        cachedRender
        && cachedRender.renderer === GRAPH_RENDERER_D3
        && cachedRender.signature === getGraphRenderSignatureForTab(tab, graphSnapshot, graphViewConfig)
        && cachedRender.wrapper
        && hasQuickAction
      );
    }

    function activateCachedGraphRender(tab, options = {}) {
      if (!canReuseGraphRender(tab)) return false;
      const cachedRender = graphRenderCache.get(tab.id);
      ...
      if (cachedRender.quickActionWrapper && !cachedRender.quickActionWrapper.isConnected) {
        cachedRender.wrapper.appendChild(cachedRender.quickActionWrapper);
      }
      hideInactiveGraphRenders(tab.id);
      cachedRender.quickActionWrapper?.classList.remove("hidden");
      if (tab.graphViewKind !== "health-report") {
        document.querySelector(".graph-view-toolbar")?.classList.remove("hidden");
        if (!options.skipToolbar) updateGraphTagToolbar(tab, tab.graphComparisonSnapshot || tab.graphSnapshot || null);
      }
      if (!options.skipToolbar && typeof renderGraphHealthPanel === "function") renderGraphHealthPanel(tab);
      tab.visiblePointCount = cachedRender.visiblePointCount || 0;
      tab.graphEdgeCount = cachedRender.graphEdgeCount || 0;
      tab.graphZoomScale = cachedRender.zoomScale || getGraphZoomScaleFromLayout(tab.graphLayout);
      tab.selectedGraphNodeCount = 0;
      tab.graphClusterCount = cachedRender.graphClusterCount || 0;
      tab.graphCollapsedNodeCount = cachedRender.graphCollapsedNodeCount || 0;
      updateStatusLine({
        visiblePointCount: tab.visiblePointCount,
        graphEdgeCount: tab.graphEdgeCount,
        graphZoomScale: tab.graphZoomScale,
        selectedGraphNodeCount: 0,
        graphClusterCount: tab.graphClusterCount,
        graphCollapsedNodeCount: tab.graphCollapsedNodeCount
      });
      return true;
    }
    ...
    const showGraphLoadingState = async (message = "Building graph view") => {
      ...
      label.className = "graph-loading-label";
    ...
    if (!activeTab || activeTab.type !== "graph") {
      ...
      if (typeof renderGraphHealthPanel === "function") renderGraphHealthPanel(null);
    ...
    renderTagManagementList();
    if (activeTab.graphViewKind !== "health-report") {
      document.querySelector(".graph-view-toolbar")?.classList.remove("hidden");
    }
    ...
    if (!options.skipToolbar && typeof renderGraphHealthPanel === "function") renderGraphHealthPanel(activeTab);
    ...
    if (activeTab.graphViewKind === "health-report") {
      ...
      document.querySelector(".graph-view-toolbar")?.classList.add("hidden");
      const healthReportSignature = JSON.stringify({
        renderer: "health-report",
        snapshot: getGraphSnapshotSignature(graphSnapshot, activeTab.graphViewConfig || null)
      });
      const healthReportRender = renderGraphHealthReportView(graphViewCanvas, activeTab, { signature: healthReportSignature });
      if (healthReportRender?.wrapper) {
        hideInactiveGraphRenders(activeTab.id);
        graphRenderCache.set(activeTab.id, {
          tabId: activeTab.id,
          renderer: "health-report",
          signature: healthReportSignature,
          wrapper: healthReportRender.wrapper,
          visiblePointCount: 0,
          graphEdgeCount: 0,
          graphClusterCount: 0,
          graphCollapsedNodeCount: 0,
          destroy: () => {
            healthReportRender.wrapper.remove();
          }
        });
      }
      activeTab.visiblePointCount = 0;
      ...
      activeTab.graphCollapsedNodeCount = 0;
      updateStatusLine({ visiblePointCount: 0, graphEdgeCount: 0, graphClusterCount: 0, graphCollapsedNodeCount: 0 });
      graphPerf?.end({ reason: "health-report", nodes: graphSnapshot.nodes?.length || 0, links: graphSnapshot.links?.length || 0 });
      return;
    }

    const graphRendererType = GRAPH_RENDERER_D3;
    const getGraphRenderSignature = () => getGraphRenderSignatureForTab(activeTab, graphSnapshot, graphViewConfig);
    ...
    if (cachedRender && cachedRender.renderer === graphRendererType && cachedRender.signature === graphSignature && cachedRender.wrapper) {
      if (activateCachedGraphRender(activeTab, options)) {
        removeGraphLoadingState();
        graphPerf?.end({
    ...
    document.querySelectorAll(`.graph-quick-action[data-graph-tab-id="${CSS.escape(String(activeTab.id))}"]`).forEach((node) => node.remove());
    ...
    const appendGraphItems = (target, items) => {
    ...
    const isMissingDependencyNode = (nodeData) => getGraphNodeType(nodeData) === "missing-dependency";
    const isExternalJarNode = (nodeData) => (
      getGraphNodeType(nodeData) === "file"
      && Array.isArray(nodeData?.tags)
      && (nodeData.tags.includes("external-jar") || nodeData.tags.includes("external-dependency"))
    );
    const isTagLink = (linkData) => getGraphLinkType(linkData) === "tag";
    const isMissingDependencyLink = (linkData) => getGraphLinkType(linkData) === "missing-dependency";
    ...
    const snapshotFileEntries = typeof getGraphSnapshotFileEntries === "function"
      ? getGraphSnapshotFileEntries(graphSnapshot)
      : (graphSnapshot.files || []);
    const snapshotFilesById = new Map(snapshotFileEntries.map((file) => [file.id, file]));
    const graphNodesById = new Map(nodes.map((node) => [node.id, node]));
    const isExternalDependencyLink = (linkData) => {
      if (isMissingDependencyLink(linkData) || isTagLink(linkData)) return false;
      const sourceNode = graphNodesById.get(getLinkSourceId(linkData));
      const targetNode = graphNodesById.get(getLinkTargetId(linkData));
      return isExternalJarNode(sourceNode) || isExternalJarNode(targetNode);
    };
    const isBulkTaggableFileNode = (nodeData) => (
      !!nodeData
      && !isTagNode(nodeData)
      && !isClusterNode(nodeData)
      && !isMissingDependencyNode(nodeData)
    );
    graphPerf?.mark("graph lookup indexes built", {
      files: snapshotFilesById.size,
      nodes: graphNodesById.size
    });
    ...
    const getGraphNodeLabel = (nodeData) => {
      ...
      if (!isClusterNode(nodeData)) {
        if (isMissingDependencyNode(nodeData)) return nodeData?.label || nodeData?.qualifiedName || "Missing dependency";
        const fileLabel = getGraphNodeFileLabelSource(nodeData);
        if (isExternalJarNode(nodeData)) return fileLabel.replace(/\.(md|markdown)$/i, "");
    ...
    const graphGroups = Array.isArray(graphViewConfig?.groups) ? graphViewConfig.groups : [];
    const needsLinkTextIndex = Boolean(graphViewConfig?.searchQuery) || graphGroups.some((group) => group && group.enabled !== false);
    const fileLinkTextById = new Map();
    if (needsLinkTextIndex) {
    ...
    graphPerf?.mark("link text index built", {
      enabled: needsLinkTextIndex,
      indexedFiles: fileLinkTextById.size,
      links: links.length
    });
    ...
    if (graphViewConfig && graphViewConfig.searchQuery) {
      ...
      nodes.length = 0;
      appendGraphItems(nodes, searchNodes);
      links.length = 0;
      appendGraphItems(links, searchLinks);
    }
    ...
    if (graphViewConfig && Array.isArray(graphViewConfig.allowedNodeIds) && graphViewConfig.allowedNodeIds.length) {
      ...
      nodes.length = 0;
      appendGraphItems(nodes, allowedNodes);
      links.length = 0;
      appendGraphItems(links, allowedLinks);
    }
    ...
    if (graphViewConfig && Array.isArray(graphViewConfig.selectedTagIds) && graphViewConfig.selectedTagIds.length) {
      ...
      nodes.length = 0;
      appendGraphItems(nodes, selectedNodes);
      links.length = 0;
      appendGraphItems(links, selectedLinks);
    }
    ...
    if (graphViewConfig && graphViewConfig.showTags === false) {
      ...
      nodes.length = 0;
      appendGraphItems(nodes, fileNodes);
      links.length = 0;
      appendGraphItems(links, nonTagLinks);
    }
    ...
    let beforeExternalNodeCount = nodes.length;
    let beforeExternalLinkCount = links.length;
    if (graphViewConfig && graphViewConfig.showExternalJars === false) {
      beforeExternalNodeCount = nodes.length;
      beforeExternalLinkCount = links.length;
      const externalJarNodeIds = new Set(nodes.filter(isExternalJarNode).map((n) => n.id));
      if (externalJarNodeIds.size) {
        const visibleNodes = nodes.filter((n) => !externalJarNodeIds.has(n.id));
        const visibleLinks = links.filter((l) => !externalJarNodeIds.has(getLinkSourceId(l)) && !externalJarNodeIds.has(getLinkTargetId(l)));
        nodes.length = 0;
        appendGraphItems(nodes, visibleNodes);
        links.length = 0;
        appendGraphItems(links, visibleLinks);
      }
    }
    graphPerf?.mark("external jar visibility filter", {
      showExternalJars: graphViewConfig?.showExternalJars !== false,
      removedNodes: graphViewConfig?.showExternalJars === false ? Math.max(0, (beforeExternalNodeCount || nodes.length) - nodes.length) : 0,
      removedLinks: graphViewConfig?.showExternalJars === false ? Math.max(0, (beforeExternalLinkCount || links.length) - links.length) : 0,
      nodes: nodes.length,
      links: links.length
    });

    if (graphViewConfig && graphViewConfig.showMissingDependencies === false) {
      const missingNodeIds = new Set(nodes.filter(isMissingDependencyNode).map((n) => n.id));
      if (missingNodeIds.size) {
        const visibleNodes = nodes.filter((n) => !missingNodeIds.has(n.id));
        const visibleLinks = links.filter((l) => !missingNodeIds.has(getLinkSourceId(l)) && !missingNodeIds.has(getLinkTargetId(l)));
        nodes.length = 0;
        appendGraphItems(nodes, visibleNodes);
        links.length = 0;
        appendGraphItems(links, visibleLinks);
    ...
    graphPerf?.mark("missing dependency visibility filter", {
      showMissingDependencies: graphViewConfig?.showMissingDependencies !== false,
      nodes: nodes.length,
      links: links.length
    });

    let hiddenGroupNodeChecks = 0;
    let hiddenGroupQueryCount = 0;
    ...
    graphPerf?.mark("hidden group filter", {
      ...
      hiddenGroupQueryCount: typeof hiddenGroupQueryCount === "number" ? hiddenGroupQueryCount : 0,
      hiddenGroupNodeChecks: typeof hiddenGroupNodeChecks === "number" ? hiddenGroupNodeChecks : 0,
      nodes: nodes.length,
      links: links.length
    });
    ...
    const filterGraphToNodeIds = (nodeIds) => {
      ...
      nodes.length = 0;
      appendGraphItems(nodes, filteredNodes);
      links.length = 0;
      appendGraphItems(links, filteredLinks);
    };
    ...
    const getFullOutgoingNodeIds = (nodeId) => {
      ...
      while (nodesToVisit.length) {
        ...
        appendGraphItems(nodesToVisit, getDirectOutgoingNodeIds(currentNodeId));
      }
    ...
    const getFullIncomingNodeIds = (nodeId) => {
      ...
      while (nodesToVisit.length) {
        ...
        appendGraphItems(nodesToVisit, getDirectIncomingNodeIds(currentNodeId));
      }
    ...
    const applyCollapsedClustersToGraph = () => {
      ...
      nodes.length = 0;
      appendGraphItems(nodes, visibleNodes);
      appendGraphItems(nodes, clusterNodes);
      links.length = 0;
      appendGraphItems(links, rewrittenLinksByKey.values());
    };
    ...
    const staticLargeGraphTickCount = useStaticLargeGraphLayout
      ? (nodes.length > 5000 || links.length > 50000
        ? (typeof getGraphStaticWarmupTicks === "function" ? getGraphStaticWarmupTicks() : 12)
        : 70)
      : 0;
    ...
    graphPerf?.mark("d3 svg created", {
      ...
      staticTickCount: staticLargeGraphTickCount
    });
    ...
    let label = null;
    let maxNodeRadius = 1;
    const isGraphLayoutPersistable = () => graphSettings.magneticEnabled === false;
    const capturePersistableGraphLayout = () => {
      if (!isGraphLayoutPersistable()) return null;
      return captureGraphLayout(activeTab, nodes, currentZoomTransform);
    };
    const savePersistableGraphLayout = () => {
      if (!capturePersistableGraphLayout()) return false;
    ...
    const zoomBehavior = d3.zoom()
      ...
      .on("zoom", (event) => {
        ...
        if (event.sourceEvent) savePersistableGraphLayout();
      });
    ...
    const link = lineLayer.selectAll("line").data(links).enter().append("line")
      .attr("class", (d) => `graph-link graph-link-${getGraphLinkType(d)}${isExternalDependencyLink(d) ? " graph-link-external-dependency" : ""} graph-link-status-${getGraphItemStatus(d)}`)
    ...
    const arrowhead = arrowheadLayer.selectAll("path").data(graphViewConfig.showArrows ? links.filter(isMarkdownLink) : []).enter().append("path")
      .attr("class", (d) => `graph-arrowhead graph-arrowhead-${getGraphLinkType(d)}${isExternalDependencyLink(d) ? " graph-arrowhead-external-dependency" : ""} graph-arrowhead-status-${getGraphItemStatus(d)}`);
    ...
    const node = nodeLayer.selectAll("circle").data(nodes).enter().append("circle")
      ...
      .style("fill", (d) => {
        ...
        if (isMissingDependencyNode(d)) return "transparent";
        if (isExternalJarNode(d)) return typeof getGraphExternalDependencyColor === "function" ? getGraphExternalDependencyColor() : "#c26a1b";
        return d.groupColor || null;
      })
      .style("stroke", (d) => isMissingDependencyNode(d) ? (typeof getGraphMissingDependencyColor === "function" ? getGraphMissingDependencyColor() : "#1a288e") : null)
      .call(d3.drag()
        .on("start", (event, d) => {
    ...
    const graphTooltipPathsById = new Map(snapshotFileEntries.map((file) => [file.id, file.fullPath || file.path]));
    const getGraphNodeTooltip = (nodeData) => {
      ...
      if (isMissingDependencyNode(nodeData)) {
        return `${nodeData.qualifiedName || getGraphNodeLabel(nodeData)} is unresolved.`;
      }
    ...
    const getSnapshotFileForNode = (graphNode) => {
      ...
      const snapshotFile = snapshotFilesById.get(graphNode.id) || activeGraphTab?.graphSnapshot?.files?.find((file) => file.id === graphNode.id) || createGraphFileDataFromNode?.(graphNode);
    ...
    const revealGraphNodeInTreeView = async (graphNode) => {
      if (!graphNode || graphNode.type === "tag") return false;
      const revealTab = getTreeRevealTabForNode(graphNode);
      const revealPaths = [
        revealTab.sourceFilePath,
        revealTab.sourceFileName,
        revealTab.title
      ].filter(Boolean);
      let treeButton = null;
      if (typeof revealFolderTreeFileByPath === "function") {
        for (const revealPath of revealPaths) {
          treeButton = await revealFolderTreeFileByPath(revealPath);
          if (treeButton) break;
        }
      }
      if (!treeButton) treeButton = findFolderTreeFileButtonForTab?.(revealTab);
    ...
    const readGraphNodeContent = async (graphNode) => {
      ...
      if (isNeutralinoRuntime() && filePath && Neutralino.filesystem?.readFile) return Neutralino.filesystem.readFile(filePath);
    ...
    const canWriteGraphNodeContent = (graphNode) => {
      const snapshotFile = getSnapshotFileForNode(graphNode);
      if (!snapshotFile) return false;
      const filePath = getNodeFilesystemPath(graphNode);
      if (isNeutralinoRuntime() && filePath && Neutralino.filesystem?.writeFile) return true;
      if (snapshotFile.handle?.createWritable) return true;
      const folderEntry = getFolderMarkdownEntryForNode(graphNode);
      return !!folderEntry?.handle?.createWritable;
    };
    ...
    const getFullOutgoingDependencyIds = (nodeId) => {
      ...
      while (nodesToVisit.length) {
        ...
        appendGraphItems(nodesToVisit, getDirectOutgoingDependencyIds(currentNodeId));
      }
    ...
    const getFullBacklinkIds = (nodeId) => {
      ...
      while (nodesToVisit.length) {
        ...
        appendGraphItems(nodesToVisit, getBacklinkIds(currentNodeId));
      }
    ...
    const getVisibleNodeIdsForTargetConfig = (snapshot, viewConfig) => {
      ...
      const getRecursiveIds = (nodeId, getNextIds) => {
        ...
        while (idsToVisit.length) {
          ...
          appendGraphItems(idsToVisit, getNextIds(currentId));
        }
        return collectedIds;
      };
    ...
    const promptForGraphPointConflict = async (sourceNode) => {
      const state = { rememberSelection: false };
      const keepBoth = await app.services.confirm({
        title: "Point Already Exists",
        message: `The target graph already has a point named "${getGraphNodeLabel(sourceNode)}".\n\nKeep Both adds the imported point with a source-specific id and keeps its connections separate.\n\nUse Existing connects the imported edges to the point that is already in the target graph.`,
        ...
        renderBody: (body) => {
          ...
          title.textContent = "Remember My Selection";
          const description = document.createElement("span");
          description.className = "settings-switch-description";
          description.textContent = "Apply this choice to all remaining point conflicts in this Add-to-Graph operation.";
          ...
          input.addEventListener("change", () => {
            state.rememberSelection = input.checked;
          });
      ...
      return { keepBoth, rememberSelection: state.rememberSelection };
    };
    const mergeGraphScopeIntoTab = async (targetTab, scope, sourceNode = contextTargetNode) => {
      ...
      const targetFileEntries = typeof getGraphSnapshotFileEntries === "function" ? getGraphSnapshotFileEntries(targetSnapshot) : (targetSnapshot.files || []);
      const targetFiles = Array.isArray(targetFileEntries) ? targetFileEntries.map((fileData) => cloneGraphValue(fileData)).filter(Boolean) : [];
      ...
      let rememberedConflictDecision = null;
      ...
      for (const sourceNode of selectedSourceNodes) {
        ...
        if (existingNode) {
          ...
          let shouldKeepBoth = rememberedConflictDecision;
          if (shouldKeepBoth === null) {
            const conflictDecision = await promptForGraphPointConflict(sourceNode);
            shouldKeepBoth = conflictDecision.keepBoth;
            if (conflictDecision.rememberSelection) rememberedConflictDecision = shouldKeepBoth;
          }
          if (!shouldKeepBoth) {
            sourceToTargetNodeId.set(sourceNode.id, nextNodeId);
            visibleImportedNodeIds.add(nextNodeId);
    ...
    const handleAddToTabScope = async (scope, scopeLabel) => {
      ...
      if (!targetTab) return;
      const didMerge = await mergeGraphScopeIntoTab(targetTab, scope, sourceNode);
    ...
    const openGraphNodeFileFromCurrentRender = async (graphNode) => {
      ...
      return openDocumentSourceFile(
        { ...sourceFile, content: normalizeEditorContent(content) },
        { temporary: false, title: getNodeEditorTitle(graphNode) }
      );
    };
    ...
    const openGraphNodeOriginalFileInNewTab = async (graphNode) => {
      ...
      return openDocumentSourceFile(sourceFile, { temporary: false, title: getFileName(sourcePath) });
    };
    ...
    const getVisibleFileNodes = () => nodes.filter((graphNode) => !isTagNode(graphNode) && !isClusterNode(graphNode) && !isMissingDependencyNode(graphNode));
    ...
    const openAllVisibleGraphFiles = async () => {
      ...
      if (fileNodes.length > 20 && (typeof shouldConfirmOpenManyGraphNodes !== "function" || shouldConfirmOpenManyGraphNodes())) {
        const shouldContinue = typeof app?.services?.confirm === "function"
          ? await app.services.confirm(`Open ${fileNodes.length} files in editor tabs?\n\nThis might slow down your computer or crash the app.`)
          : window.confirm(`Open ${fileNodes.length} files in editor tabs?\n\nThis might slow down your computer or crash the app.`);
    ...
    const createOriginalExportLogger = (destinationFolder) => {
      const preferences = typeof getDebugPreferences === "function" ? getDebugPreferences() : { enabled: false, writeToFile: false, logPath: "" };
      const logFilePath = preferences.enabled && preferences.writeToFile ? preferences.logPath : "";

      const log = async (message, details, level = "debug") => {
        if (typeof appDebugLog !== "function") return null;
        return appDebugLog(level, `[original-export] ${message}`, details);
      };

      log("Original graph export started", { destinationFolder }, "info");
      return { log, logFilePath };
    };

    const getDependentExternalJarNodes = (sourceNodes) => {
      const dependencyClosure = deps?.jarDependencyClosure || globalThis.MdEditorJarDependencyClosure;
      if (typeof dependencyClosure?.collectExternalJarDependencyClosure === "function") {
        return dependencyClosure.collectExternalJarDependencyClosure({
          sourceNodes,
          links: graphSnapshot?.links || [],
          getSourceId: getGraphLinkSourceId,
          getTargetId: getGraphLinkTargetId,
          getNodeById: (nodeId) => graphNodesById.get(nodeId),
          isExternalJarNode
        }).nodes;
      }
      const sourceIds = new Set((sourceNodes || []).map((nodeData) => nodeData?.id).filter(Boolean));
      const jarNodeIds = new Set();
      (graphSnapshot?.links || []).forEach((linkData) => {
        const sourceId = getGraphLinkSourceId(linkData);
        const targetId = getGraphLinkTargetId(linkData);
        if (!sourceIds.has(sourceId)) return;
        const targetNode = graphNodesById.get(targetId);
        if (isExternalJarNode(targetNode)) jarNodeIds.add(targetId);
      });
      return Array.from(jarNodeIds)
        .map((nodeId) => graphNodesById.get(nodeId))
        .filter(Boolean);
    };

    const getUniqueJarExportRelativePath = (jarNode, usedNames) => {
      const sourceName = getNodeFileName(jarNode?.id || jarNode?.path || jarNode?.label || "external.jar")
        .replace(/\.md$/i, "");
      const baseName = sourceName.endsWith(".jar") ? sourceName : `${sourceName}.jar`;
      let candidate = `lib/${baseName}`;
      let index = 2;
      while (usedNames.has(candidate.toLowerCase())) {
        candidate = `lib/${baseName.replace(/\.jar$/i, "")}-${index}.jar`;
        index += 1;
      }
    ...
    const copyOriginalBinaryFile = async (sourcePath, destinationPath, logExportStep) => {
      if (Neutralino.filesystem?.copy) {
        try {
          if (logExportStep) await logExportStep("JAR filesystem.copy before", { sourcePath, destinationPath });
          await Neutralino.filesystem.copy(sourcePath, destinationPath);
          if (logExportStep) await logExportStep("JAR filesystem.copy after", { sourcePath, destinationPath });
          return;
        } catch (error) {
          if (logExportStep) await logExportStep("JAR filesystem.copy failed; falling back to binary write", { sourcePath, destinationPath, error: error?.message || String(error) });
        }
      }
      if (Neutralino.filesystem?.readBinaryFile && Neutralino.filesystem?.writeBinaryFile) {
        if (logExportStep) await logExportStep("JAR readBinaryFile before", { sourcePath });
        const data = await Neutralino.filesystem.readBinaryFile(sourcePath);
        if (logExportStep) await logExportStep("JAR readBinaryFile after", { sourcePath, byteLength: data?.byteLength ?? data?.length ?? null });
        if (logExportStep) await logExportStep("JAR writeBinaryFile before", { destinationPath });
        await Neutralino.filesystem.writeBinaryFile(destinationPath, data);
        if (logExportStep) await logExportStep("JAR writeBinaryFile after", { destinationPath });
        return;
      }
      if (logExportStep) await logExportStep("JAR binary methods unavailable; falling back to text copy", { sourcePath, destinationPath });
      await Neutralino.filesystem.writeFile(destinationPath, await Neutralino.filesystem.readFile(sourcePath));
    };

    const isJavaOriginalSourcePath = (path) => /\.java$/i.test(String(path || "").trim());

    const JAVA_ORIGINAL_EXPORT_COMPILE_BATCH = [
      "@echo off",
      "",
    ...
    const writeJavaOriginalExportCompileBatch = async (destinationFolder, logExportStep) => {
      const compileBatchPath = joinPath(destinationFolder, "compile.bat");
      if (logExportStep) await logExportStep("Java compile batch write before", { compileBatchPath });
      await Neutralino.filesystem.writeFile(compileBatchPath, JAVA_ORIGINAL_EXPORT_COMPILE_BATCH);
      if (logExportStep) await logExportStep("Java compile batch write after", { compileBatchPath });
    };

    const DEFAULT_ORIGINAL_EXPORT_OPTIONS = Object.freeze({
      createClasspath: true,
      createCompileBat: true,
      includeDependencyJars: true
    });

    const buildJavaOriginalExportClasspath = (exportedJars = []) => [
      '<?xml version="1.0" encoding="UTF-8"?>',
      "<classpath>",
      '  <classpathentry kind="src" path="src"/>',
      ...exportedJars.map((relativePath) => `  <classpathentry kind="lib" path="${xmlEscape(relativePath)}"/>`),
      '  <classpathentry kind="output" path="bin"/>',
      "</classpath>"
    ].join("\n");

    const writeJavaOriginalExportClasspath = async (destinationFolder, exportedJars, logExportStep) => {
      const classpath = buildJavaOriginalExportClasspath(exportedJars);
      const classpathPath = joinPath(destinationFolder, ".classpath");
      if (logExportStep) await logExportStep(".classpath write before", { classpathPath, jarCount: exportedJars.length });
      await Neutralino.filesystem.writeFile(classpathPath, classpath);
      if (logExportStep) await logExportStep(".classpath write after", { classpathPath });
    };

    const getOriginalExportOptionsMessage = (dependentJarCount) => {
      if (dependentJarCount > 0) {
        return `This export depends on ${dependentJarCount} external JAR file${dependentJarCount === 1 ? "" : "s"}.`;
      }
      return "Choose what to create for this export.";
    };

    const createOriginalExportOptionRow = (inputId, labelText, checked) => {
      const row = document.createElement("label");
      row.className = "settings-switch-row original-export-option-row";
      ...
      row.append(textWrap, input, switchElement);
      return { row, input };
    };

    const renderOriginalExportOptionsControls = (body, defaults = DEFAULT_ORIGINAL_EXPORT_OPTIONS) => {
      const container = document.createElement("div");
      container.className = "original-export-options";
      const classpathOption = createOriginalExportOptionRow("original-export-create-classpath", "Create .classpath", defaults.createClasspath);
      const compileBatOption = createOriginalExportOptionRow("original-export-create-compile-bat", "Create compile.bat file", defaults.createCompileBat);
      const jarsOption = createOriginalExportOptionRow("original-export-include-dependency-jars", "Include dependency JAR files in lib", defaults.includeDependencyJars);
      container.append(classpathOption.row, compileBatOption.row, jarsOption.row);
      body.appendChild(container);
      return {
        createClasspathInput: classpathOption.input,
        createCompileBatInput: compileBatOption.input,
        includeDependencyJarsInput: jarsOption.input
      };
    };

    const getOriginalExportOptionsFromInputs = (inputs) => ({
      createClasspath: !!inputs?.createClasspathInput?.checked,
      createCompileBat: !!inputs?.createCompileBatInput?.checked,
      includeDependencyJars: !!inputs?.includeDependencyJarsInput?.checked
    });

    const showOriginalExportOptionsFallbackDialog = ({ dependentJarCount }) => new Promise((resolve) => {
      const overlay = document.createElement("div");
      overlay.className = "reset-modal-overlay original-export-options-overlay";
      overlay.setAttribute("role", "dialog");
      overlay.setAttribute("aria-modal", "true");
      overlay.setAttribute("aria-labelledby", "original-export-options-title");
      overlay.style.display = "flex";

      const box = document.createElement("div");
      box.className = "reset-modal-box original-export-options-box";

      const title = document.createElement("p");
      title.id = "original-export-options-title";
      title.className = "reset-modal-message";
      title.textContent = "Export Originals";

      const message = document.createElement("div");
      message.className = "app-notification-message";
      message.textContent = getOriginalExportOptionsMessage(dependentJarCount);

      const body = document.createElement("div");
      body.className = "app-notification-body";
      const inputs = renderOriginalExportOptionsControls(body);

      const actions = document.createElement("div");
      actions.className = "reset-modal-actions original-export-options-actions";
      ...
      const okButton = document.createElement("button");
      okButton.type = "button";
      okButton.className = "reset-modal-btn settings-primary-action";
      okButton.textContent = "OK";
      ...
      okButton.addEventListener("click", () => closeDialog(getOriginalExportOptionsFromInputs(inputs)));
    ...
    const showOriginalExportOptionsDialog = async ({ dependentJarCount }) => {
      let inputs = null;
      if (typeof app?.services?.notify?.show === "function") {
        const result = await app.services.notify.show({
          title: "Export Originals",
          message: getOriginalExportOptionsMessage(dependentJarCount),
          dismissValue: null,
          buttons: [
            { id: "cancel", label: "Cancel", value: null, variant: "cancel" },
            { id: "confirm", label: "OK", variant: "primary", autoFocus: true, action: () => getOriginalExportOptionsFromInputs(inputs) }
          ],
          renderBody(body) {
            inputs = renderOriginalExportOptionsControls(body);
          }
        });
        return result && typeof result === "object" ? result : null;
      }
      return showOriginalExportOptionsFallbackDialog({ dependentJarCount });
    };

    const cleanupExportedJavaSourceContent = async (sourceContent, sourcePath, destinationPath, logExportStep) => {
      if (!isJavaOriginalSourcePath(sourcePath) && !isJavaOriginalSourcePath(destinationPath)) {
        return { content: sourceContent, removedImportCount: 0 };
      }
      if (!javaImportCleanup?.cleanupJavaUnusedImports) {
        if (logExportStep) await logExportStep("Java import cleanup unavailable", { sourcePath, destinationPath }, "warning");
        return { content: sourceContent, removedImportCount: 0 };
      }
      const cleanupResult = javaImportCleanup.cleanupJavaUnusedImports(sourceContent, { sourcePath, destinationPath });
      if (cleanupResult?.skipped) {
        if (logExportStep) await logExportStep("Java import cleanup skipped", {
          sourcePath,
          destinationPath,
          reason: cleanupResult.reason,
          unsupportedImportLine: cleanupResult.unsupportedImportLine || ""
        }, "warning");
        return { content: sourceContent, removedImportCount: 0 };
      }
      const removedImportCount = Number(cleanupResult?.removedImportCount || 0);
      if (logExportStep) await logExportStep("Java import cleanup finished", {
        sourcePath,
        destinationPath,
        importCount: Number(cleanupResult?.importCount || 0),
        removedImportCount,
        changed: !!cleanupResult?.changed
      }, removedImportCount ? "info" : "debug");
      return {
        content: cleanupResult?.content ?? sourceContent,
        removedImportCount
      };
    };

    const exportDependentExternalJars = async (jarNodes, destinationFolder, createdDirectories, logExportStep) => {
      const exportedJars = [];
      const failedJars = [];
      const usedNames = new Set();

      if (logExportStep) await logExportStep("External JAR export started", { count: jarNodes.length });
      for (const jarNode of jarNodes) {
        const jarName = getNodeFileName(jarNode.id);
        try {
          if (logExportStep) await logExportStep("External JAR markdown read before", { jar: jarName, nodeId: jarNode.id });
          const markdown = await readGraphNodeContent(jarNode);
          if (logExportStep) await logExportStep("External JAR markdown read after", { jar: jarName });
          const sourcePath = getSourceFilePathFromMarkdown(markdown);
          if (!sourcePath) {
            if (logExportStep) await logExportStep("External JAR missing source_file", { jar: jarName }, "warning");
            failedJars.push(`${jarName}: missing source_file`);
            continue;
          }
          const relativePath = getUniqueJarExportRelativePath(jarNode, usedNames);
          const destinationPath = joinPath(destinationFolder, relativePath);
          if (logExportStep) await logExportStep("External JAR export path resolved", { jar: jarName, sourcePath, destinationPath });
          if (Neutralino.filesystem?.getStats) {
            if (logExportStep) await logExportStep("External JAR source getStats before", { jar: jarName, sourcePath });
            const sourceStats = await Neutralino.filesystem.getStats(sourcePath);
            if (logExportStep) await logExportStep("External JAR source getStats after", { jar: jarName, sourcePath, stats: sourceStats });
          }
          if (logExportStep) await logExportStep("External JAR destination directories before", { jar: jarName, destinationPath });
          await ensureOriginalExportDirectories(destinationPath, createdDirectories);
          if (logExportStep) await logExportStep("External JAR destination directories after", { jar: jarName, destinationPath });
          await copyOriginalBinaryFile(sourcePath, destinationPath, logExportStep);
          if (Neutralino.filesystem?.getStats) {
            if (logExportStep) await logExportStep("External JAR destination getStats before", { jar: jarName, destinationPath });
            const destinationStats = await Neutralino.filesystem.getStats(destinationPath);
            if (logExportStep) await logExportStep("External JAR destination getStats after", { jar: jarName, destinationPath, stats: destinationStats });
          }
          exportedJars.push(relativePath);
          if (logExportStep) await logExportStep("External JAR export succeeded", { jar: jarName, relativePath });
        } catch (error) {
          if (logExportStep) await logExportStep("External JAR export failed", { jar: jarName, error: error?.message || String(error) }, "error");
          failedJars.push(`${jarName}: ${error?.message || "export failed"}`);
        }
      }

      if (logExportStep) await logExportStep("External JAR export finished", { exported: exportedJars.length, failed: failedJars.length }, failedJars.length ? "warning" : "info");
      return { exportedJars, failedJars };
    };
    ...
    const showOriginalExportSummaryDialog = async ({
      destinationFolder,
      sourceTotal,
      exportedSourceCount,
      jarTotal,
      ...
      exportLogger,
      abortedError,
      removedJavaImportCount = 0
    }) => {
      const message = [
        `Exported ${exportedSourceCount} original file${exportedSourceCount === 1 ? "" : "s"} out of ${sourceTotal}.`
      ];
      if (removedJavaImportCount > 0) {
        message.push(`Removed ${removedJavaImportCount} unused Java import${removedJavaImportCount === 1 ? "" : "s"} from exported source copies.`);
      }
      if (jarTotal > 0) {
        message.push(`Exported ${exportedJarCount} external JAR${exportedJarCount === 1 ? "" : "s"} out of ${jarTotal}.`);
      }
      if (abortedError) {
        message.push(`Export stopped with an error: ${abortedError?.message || abortedError}`);
      }
      if (failedItems.length) {
        message.push(`Skipped ${failedItems.length} item${failedItems.length === 1 ? "" : "s"}:\n${failedItems.slice(0, 10).join("\n")}`);
      }
      if (exportLogger?.logFilePath) {
        message.push(`Debug log: ${exportLogger.logFilePath}`);
      } else if (abortedError || failedItems.length) {
        message.push("Debug logging is disabled or no debug log path is configured.");
      }
      await showOriginalExportCompleteDialog(message.join("\n\n"), destinationFolder);
    };

    const originalExportStatusId = `graph-original-export:${activeTab.id}`;

    /** Display the current original-file export stage in the graph and global process UI. */
    const setOriginalExportProgress = (message) => {
      const label = String(message || "Exporting original files...");
      setGraphQuickActionBusy(true, label);
      app.modules?.statusManager?.setStatus?.({
        id: originalExportStatusId,
        label,
        showProgress: true,
        priority: 1,
        backgroundProcess: {
          category: "Graph export",
          icon: "bi-box-arrow-up-right"
        }
      });
    };

    /** Clear the original-file export activity from the graph and global process UI. */
    const clearOriginalExportProgress = (outcome = "finished") => {
      setGraphQuickActionBusy(false);
      app.modules?.statusManager?.unsetStatus?.(originalExportStatusId, {
        outcome,
        description: outcome === "failed" ? "Original graph export completed with errors" : "Original graph export completed"
      });
    };
    ...
    const exportOriginalGraphNodes = async () => {
      ...
      const sourceFileNodes = fileNodes.filter((graphNode) => !isExternalJarNode(graphNode));
      if (!sourceFileNodes.length) {
        alert("There are no visible source file points to export.");
        return;
      }
      const dependentJarNodes = getDependentExternalJarNodes(sourceFileNodes);
      const exportOptions = await showOriginalExportOptionsDialog({ dependentJarCount: dependentJarNodes.length });
      if (!exportOptions) return;

      const destinationFolder = await Neutralino.os.showFolderDialog("Select destination folder");
      if (!destinationFolder) return;

      const exportLogger = createOriginalExportLogger(destinationFolder);
      const logExportStep = exportLogger.log;
      await logExportStep("Visible export nodes resolved", {
        visibleFileNodes: fileNodes.length,
        sourceFileNodes: sourceFileNodes.length,
        dependentJarNodes: dependentJarNodes.length
      }, "info");
      await logExportStep("Original export options selected", exportOptions, "info");

      const createdDirectories = new Set();
      const exportedPaths = [];
      const failedItems = [];
      let jarExportResult = { exportedJars: [], failedJars: [] };
      let abortedError = null;
      let removedJavaImportCount = 0;
      let exportedJavaSourceCount = 0;
      const originalExportStartedAt = Date.now();
      const originalExportTimings = {
        markdownReadMs: 0,
      ...
      const slowestSourceExports = [];

      setOriginalExportProgress(`Exporting original files 0 / ${sourceFileNodes.length}`);
      await new Promise((resolve) => requestAnimationFrame(resolve));

      try {
        for (const [sourceIndex, graphNode] of sourceFileNodes.entries()) {
          setOriginalExportProgress(`Exporting original files ${sourceIndex + 1} / ${sourceFileNodes.length}`);
          const nodeName = getNodeFileName(graphNode.id);
          const sourceNodeStartedAt = Date.now();
          try {
            await logExportStep("Source node markdown read before", { node: nodeName, nodeId: graphNode.id });
            const markdownReadStartedAt = Date.now();
            const markdown = await readGraphNodeContent(graphNode);
            const markdownReadMs = Date.now() - markdownReadStartedAt;
            originalExportTimings.markdownReadMs += markdownReadMs;
            await logExportStep("Source node markdown read after", { node: nodeName, durationMs: markdownReadMs });
            const sourcePath = getSourceFilePathFromMarkdown(markdown);
            if (!sourcePath) {
              await logExportStep("Source node missing source_file", { node: nodeName }, "warning");
              failedItems.push(`${nodeName}: missing source_file`);
              continue;
            }
            const sourceResolutionStartedAt = Date.now();
            const resolvedSource = await resolveOriginalSourcePath(sourcePath);
            originalExportTimings.sourceResolutionMs += Date.now() - sourceResolutionStartedAt;
            if (resolvedSource?.needsSourceRoot || !resolvedSource?.resolvedPath) {
              await logExportStep("Source node missing source root", { node: nodeName, sourcePath }, "warning");
              failedItems.push(`${nodeName}: original source root not set`);
              continue;
            }

            const relativeExportPath = getOriginalExportRelativePath(resolvedSource.resolvedPath, graphNode);
            if (!relativeExportPath) {
              await logExportStep("Source node export path failed", { node: nodeName, sourcePath }, "warning");
              failedItems.push(`${nodeName}: unable to derive export path`);
              continue;
            }

            await logExportStep("Source file read before", { node: nodeName, sourcePath: resolvedSource.resolvedPath });
            const sourceReadStartedAt = Date.now();
            const sourceContent = await Neutralino.filesystem.readFile(resolvedSource.resolvedPath);
            const sourceReadMs = Date.now() - sourceReadStartedAt;
            originalExportTimings.sourceReadMs += sourceReadMs;
            await logExportStep("Source file read after", { node: nodeName, sourcePath: resolvedSource.resolvedPath, length: sourceContent?.length ?? null, durationMs: sourceReadMs });
            const destinationPath = joinPath(destinationFolder, relativeExportPath);
            await logExportStep("Source destination directories before", { node: nodeName, destinationPath });
            const directoryStartedAt = Date.now();
            await ensureOriginalExportDirectories(destinationPath, createdDirectories);
            const directoryMs = Date.now() - directoryStartedAt;
            originalExportTimings.directoryMs += directoryMs;
            await logExportStep("Source destination directories after", { node: nodeName, destinationPath, durationMs: directoryMs });
            const cleanupStartedAt = Date.now();
            const cleanedSource = await cleanupExportedJavaSourceContent(sourceContent, resolvedSource.resolvedPath, destinationPath, logExportStep);
            originalExportTimings.cleanupMs += Date.now() - cleanupStartedAt;
            removedJavaImportCount += cleanedSource.removedImportCount;
            await logExportStep("Source writeFile before", { node: nodeName, destinationPath });
            const sourceWriteStartedAt = Date.now();
            await Neutralino.filesystem.writeFile(destinationPath, cleanedSource.content);
            const sourceWriteMs = Date.now() - sourceWriteStartedAt;
            originalExportTimings.sourceWriteMs += sourceWriteMs;
            const sourceNodeDurationMs = Date.now() - sourceNodeStartedAt;
            slowestSourceExports.push({ node: nodeName, durationMs: sourceNodeDurationMs });
            slowestSourceExports.sort((left, right) => right.durationMs - left.durationMs);
            if (slowestSourceExports.length > 10) slowestSourceExports.length = 10;
            await logExportStep("Source writeFile after", { node: nodeName, destinationPath, durationMs: sourceWriteMs, sourceNodeDurationMs });
            exportedPaths.push(destinationPath);
            if (isJavaOriginalSourcePath(resolvedSource.resolvedPath) || isJavaOriginalSourcePath(destinationPath)) {
              exportedJavaSourceCount += 1;
            }
            if ((sourceIndex + 1) % 50 === 0 || sourceIndex + 1 === sourceFileNodes.length) {
              const elapsedMs = Date.now() - originalExportStartedAt;
              await logExportStep("Source export timing progress", {
                processedSources: sourceIndex + 1,
                totalSources: sourceFileNodes.length,
                elapsedMs,
                averageSourceMs: Math.round(elapsedMs / (sourceIndex + 1)),
                phaseTotalsMs: { ...originalExportTimings }
              }, "info");
            }
          } catch (error) {
            await logExportStep("Source node export failed", { node: nodeName, error: error?.message || String(error) }, "error");
            failedItems.push(`${nodeName}: ${error?.message || "export failed"}`);
          }
        }

        if (exportedJavaSourceCount > 0 && exportOptions.createCompileBat) {
          await writeJavaOriginalExportCompileBatch(destinationFolder, logExportStep);
        } else if (exportedJavaSourceCount > 0) {
          await logExportStep("User skipped Java compile batch export", { exportedJavaSourceCount }, "info");
        }

        if (dependentJarNodes.length && exportOptions.includeDependencyJars) {
          try {
            setOriginalExportProgress(`Exporting dependency JARs 0 / ${dependentJarNodes.length}`);
            await logExportStep("User selected external JAR export", { dependentJarNodes: dependentJarNodes.length }, "info");
            jarExportResult = await exportDependentExternalJars(dependentJarNodes, destinationFolder, createdDirectories, logExportStep);
            appendGraphItems(failedItems, jarExportResult.failedJars);
          } catch (error) {
            await logExportStep("External JAR export aborted", { error: error?.message || String(error) }, "error");
            failedItems.push(`External JAR export: ${error?.message || "export failed"}`);
          }
        } else if (dependentJarNodes.length) {
          await logExportStep("User skipped external JAR export", { dependentJarNodes: dependentJarNodes.length }, "info");
        } else {
          await logExportStep("No dependent external JARs detected", undefined, "info");
        }

        if (exportedJavaSourceCount > 0 && exportOptions.createClasspath) {
          try {
            await writeJavaOriginalExportClasspath(destinationFolder, jarExportResult.exportedJars, logExportStep);
          } catch (error) {
            await logExportStep(".classpath write failed", { error: error?.message || String(error) }, "error");
            failedItems.push(`.classpath: ${error?.message || "write failed"}`);
          }
        } else if (exportedJavaSourceCount > 0) {
          await logExportStep("User skipped .classpath export", { exportedJavaSourceCount }, "info");
        }
      } catch (error) {
        abortedError = error;
        await logExportStep("Original graph export crashed", { error: error?.message || String(error) }, "error");
      } finally {
        await logExportStep("Original graph export summary", {
          exportedSources: exportedPaths.length,
          totalSources: sourceFileNodes.length,
          exportedJavaSources: exportedJavaSourceCount,
          exportedJars: jarExportResult.exportedJars.length,
          totalJars: dependentJarNodes.length,
          removedJavaImports: removedJavaImportCount,
          failures: failedItems.length,
          aborted: !!abortedError,
          durationMs: Date.now() - originalExportStartedAt,
          sourcePhaseTotalsMs: { ...originalExportTimings },
          slowestSourceExports
        }, abortedError || failedItems.length ? "warning" : "info");

        clearOriginalExportProgress(abortedError || failedItems.length ? "failed" : "finished");
        await showOriginalExportSummaryDialog({
          destinationFolder,
          sourceTotal: sourceFileNodes.length,
          exportedSourceCount: exportedPaths.length,
          jarTotal: dependentJarNodes.length,
          exportedJarCount: jarExportResult.exportedJars.length,
          failedItems,
          exportLogger,
          abortedError,
          removedJavaImportCount
        });
      }
    };
    ...
    const exportOriginalGraphNode = async (graphNode) => {
      ...
      try {
        ...
        let removedJavaImportCount = 0;
        if (isExternalJarNode(graphNode)) {
          await copyOriginalBinaryFile(resolvedSource.resolvedPath, destinationPath);
        } else {
          const sourceContent = await Neutralino.filesystem.readFile(resolvedSource.resolvedPath);
          const cleanedSource = await cleanupExportedJavaSourceContent(sourceContent, resolvedSource.resolvedPath, destinationPath, null);
          removedJavaImportCount = cleanedSource.removedImportCount;
          await Neutralino.filesystem.writeFile(destinationPath, cleanedSource.content);
          if (isJavaOriginalSourcePath(resolvedSource.resolvedPath) || isJavaOriginalSourcePath(destinationPath)) {
            await writeJavaOriginalExportCompileBatch(destinationFolder, null);
          }
        }
        const message = removedJavaImportCount > 0
          ? `Exported 1 original file.` + "\n\n" + `Removed ${removedJavaImportCount} unused Java import${removedJavaImportCount === 1 ? "" : "s"} from the exported source copy.`
          : "Exported 1 original file.";
        await showOriginalExportCompleteDialog(message, destinationFolder);
      } catch (error) {
    ...
    const hideGraphPoint = (graphNode) => {
      return withPausedGraphDraftPersistence("graph:hide-point", () => {
    ...
    const hideLeafGraphPoints = () => {
      return withPausedGraphDraftPersistence("graph:remove-leaf-nodes", () => {
    ...
    const updateGraphCollapsedClusters = (collapsedClusters) => {
      return withPausedGraphDraftPersistence("graph:collapsed-clusters", () => {
      simulation.stop();
      const activeGraphTab = getActiveGraphTab();
    ...
    const tagGraphNodesAndCreateGroup = async (selectedNodes, normalizedTag, options = {}) => {
      ...
      try {
        ...
        const failedNodes = [];
        const formatFailedNodeReport = () => {
          if (!failedNodes.length) return "";
          const maxShown = 25;
          const lines = failedNodes.slice(0, maxShown).map((failure, index) => {
            const nodeLabel = getGraphNodeLabel(failure.node) || failure.node?.id || `File ${index + 1}`;
            const message = failure.error?.message || String(failure.error || "Unknown error");
            return `${index + 1}. ${nodeLabel}: ${message}`;
          });
          const remaining = failedNodes.length - lines.length;
          return [
            "",
            `Failed ${failureNoun}${failedNodes.length === 1 ? "" : "s"}:`,
            ...lines,
            remaining > 0 ? `... and ${remaining} more. See the debug log/console for full details.` : ""
          ].filter(Boolean).join("\n");
        };
        const changedSnapshotFiles = [];
        for (let index = 0; index < selectedNodes.length; index += 1) {
          const graphNode = selectedNodes[index];
          setGraphQuickActionBusy(true, `${progressVerb} file ${index + 1} / ${selectedNodes.length}`, { failedCount: batchTotals.failed });
          ...
          try {
            ...
            failedNodes.push({ node: graphNode, error });
        ...
        setGraphQuickActionBusy(true, groupMessage, { failedCount: batchTotals.failed });
        ...
        if (changedSnapshotFiles.length) {
          setGraphQuickActionBusy(true, `Rebuilding graph snapshot for ${changedSnapshotFiles.length} tagged file${changedSnapshotFiles.length === 1 ? "" : "s"}...`, { failedCount: batchTotals.failed });
        ...
        if (!changedFiles && !changedGroups) {
          if (failedNodes.length) {
            void appDebugLog?.("warning", "[graph] Bulk tag operation failed for all selected files", {
              tag: normalizedTag,
              failed: failedNodes.map((failure) => ({
                nodeId: failure.node?.id || "",
                label: getGraphNodeLabel(failure.node),
                message: failure.error?.message || String(failure.error || "")
              }))
            });
            alert(`Unable to tag ${failedNodes.length} ${failureNoun}${failedNodes.length === 1 ? "" : "s"}.${formatFailedNodeReport()}`);
          }
          return;
        }
        ...
        setGraphQuickActionBusy(true, "Refreshing graph...", { failedCount: batchTotals.failed });
        ...
        tagPerf?.mark("final refresh complete", {
          refreshMs: elapsedGraphPerfMs(refreshStart),
          failed: failedNodes.length
        });

        if (failedNodes.length) {
          void appDebugLog?.("warning", "[graph] Bulk tag operation completed with failed files", {
            tag: normalizedTag,
          ...
          alert(`Tagged ${selectedNodes.length - failedNodes.length} ${failureNoun}${selectedNodes.length - failedNodes.length === 1 ? "" : "s"}, but ${failedNodes.length} file${failedNodes.length === 1 ? "" : "s"} could not be updated.${formatFailedNodeReport()}`);
        }
      } finally {
    ...
    const groupMostReferencedGraphNodes = async () => {
      ...
      const tagName = await promptForNewTag({
        title: "Tag most referenced files",
        message: "Enter a tag name for the most referenced files.",
        defaultValue: "infrastructure",
        confirmLabel: "Tag files"
      });
      const normalizedTag = normalizeTagName(tagName);
      if (!normalizedTag) return;
      const detectionPerf = typeof createGraphPerfSession === "function"
        ? createGraphPerfSession("group most referenced detection", {
          tabId: activeGraphTab.id,
          title: activeGraphTab.title || "",
          percent: getGraphMostReferencedPercent(),
          snapshotNodes: graphSnapshot.nodes?.length || 0,
          snapshotLinks: graphSnapshot.links?.length || 0
        })
        : null;
      const detectionStart = nowForGraphPerf();
      const selectedNodes = getMostReferencedGraphNodes();
      detectionPerf?.mark("rank referenced nodes", {
        durationMs: elapsedGraphPerfMs(detectionStart),
        selectedFiles: selectedNodes.length
      });
      detectionPerf?.end({ selectedFiles: selectedNodes.length });
      await withPausedGraphDraftPersistence("graph:group-most-referenced", () => tagGraphNodesAndCreateGroup(selectedNodes, normalizedTag, {
    ...
    const getUngroupedGraphNodes = () => nodes
      .filter((node) => isBulkTaggableFileNode(node) && !getMatchingGraphGroupForNode(node));
    ...
    const groupAllUngroupedGraphNodes = async () => {
      ...
      const tagName = await promptForNewTag({
        title: "Group ungrouped files",
        message: "Enter a tag name for all ungrouped files.",
        defaultValue: "the_rest",
        confirmLabel: "Group files"
      });
      const normalizedTag = normalizeTagName(tagName);
      if (!normalizedTag) return;
      const selectedNodes = getUngroupedGraphNodes();
      await withPausedGraphDraftPersistence("graph:group-all-ungrouped", () => tagGraphNodesAndCreateGroup(selectedNodes, normalizedTag, {
    ...
    const removeGraphPointFromSnapshot = (nodeId) => {
      return withPausedGraphDraftPersistence("graph:remove-point", () => {
      const activeGraphTab = getActiveGraphTab();
      ...
      activeGraphTab.graphSnapshot = {
        ...
        files: Array.isArray(activeGraphTab.graphSnapshot.files)
          ? activeGraphTab.graphSnapshot.files.filter((file) => file.id !== nodeId)
          : undefined
      };
      activeGraphTab.graphViewConfig = {
        ...(activeGraphTab.graphViewConfig || {}),
    ...
    const rebuildOpenGraphSnapshotsAfterTagChange = async (changedSnapshotFile) => {
      ...
      for (const tab of tabs) {
        if (tab?.type !== "graph" || !tab.graphSnapshot || isKeepSavedGraphMode(tab)) continue;

        let graphChanged = false;
        const snapshotEntries = typeof getGraphSnapshotFileEntries === "function" ? getGraphSnapshotFileEntries(tab.graphSnapshot) : (tab.graphSnapshot.files || []);
        snapshotEntries.forEach((snapshotFile) => {
          ...
          const sourceNode = (tab.graphSnapshot.nodes || []).find((node) => node.id === snapshotFile.id);
          if (sourceNode) sourceNode.tags = snapshotFile.tags;
          graphChanged = true;
        });

        if (!graphChanged) continue;
        const currentSnapshot = tab.graphSnapshot;
        tab.graphSnapshot = await createGraphSnapshot(snapshotEntries || [], currentSnapshot.folderName || tab.folderName || tab.title);
    ...
    const updateGraphNodeTagContent = async (graphNode, tag, action, options = {}) => {
      ...
      const snapshotFile = getSnapshotFileForNode(graphNode);
      if (isKeepSavedGraphMode(activeGraphTab)) {
        const message = "Saved graph mode does not update saved tags or links.";
        if (options.suppressAlerts) throw new Error(message);
        alert(message);
        return false;
      }
      if (!activeGraphTab || !snapshotFile) {
        const message = "Unable to find the selected file in this graph snapshot.";
        if (options.suppressAlerts) throw new Error(message);
        alert(message);
        return false;
      }

      const allowSnapshotTagFallback = options.allowSnapshotTagFallback === true;
      const shouldUpdateSnapshotOnly = allowSnapshotTagFallback && !canWriteGraphNodeContent(graphNode);
      let perfStepStart = nowForGraphPerf();
      let currentContent = "";
      if (shouldUpdateSnapshotOnly) {
        currentContent = typeof snapshotFile.content === "string" ? snapshotFile.content : "";
      } else {
        currentContent = await readGraphNodeContent(graphNode);
      }
      if (perfTimings) perfTimings.readMs = elapsedGraphPerfMs(perfStepStart);

      perfStepStart = nowForGraphPerf();
      const currentTags = shouldUpdateSnapshotOnly
        ? normalizeFileTagList([
          ...getFileTagsFromContent(currentContent),
          ...(Array.isArray(graphNode.tags) ? graphNode.tags : []),
          ...(Array.isArray(snapshotFile.tags) ? snapshotFile.tags : [])
        ])
        : getFileTagsFromContent(currentContent);
      ...
      const nextTags = action === "remove"
        ? currentTags.filter((tagName) => tagName !== normalizedTag)
        : normalizeFileTagList([...currentTags, normalizedTag]);
      ...
      perfStepStart = nowForGraphPerf();
      if (!shouldUpdateSnapshotOnly && contentChanged) await writeGraphNodeContent(graphNode, nextContent);
      if (perfTimings) perfTimings.writeMs = elapsedGraphPerfMs(perfStepStart);

      perfStepStart = nowForGraphPerf();
      if (!shouldUpdateSnapshotOnly || typeof snapshotFile.content === "string") snapshotFile.content = nextContent;
      snapshotFile.tags = nextTags;
      graphNode.tags = snapshotFile.tags;
      ...
      if (openMarkdownTab) {
        ...
        if (openMarkdownTab.id === activeTabId) {
          if (activeEditorCommands?.setActiveEditorValue) {
            activeEditorCommands.setActiveEditorValue(normalizedContent);
          } else {
    ...
    const updateLocalGraphTags = async (seedNode, tag, action, mode = "local") => {
      ...
      return withPausedGraphDraftPersistence(`graph:tag-${mode}`, async () => {
    ...
    const quickActionFailureStatus = document.createElement("div");
    quickActionFailureStatus.className = "graph-quick-action-failures hidden";
    quickActionFailureStatus.setAttribute("role", "status");
    quickActionFailureStatus.setAttribute("aria-live", "polite");
    quickActionFailureStatus.textContent = "";
    ...
    const healthReportButton = document.createElement("button");
    healthReportButton.type = "button";
    healthReportButton.className = "graph-quick-action-menu-item";
    healthReportButton.setAttribute("role", "menuitem");
    healthReportButton.innerHTML = '<i class="bi bi-clipboard2-pulse" aria-hidden="true"></i><span>Show Health graph report</span>';
    ...
    const exportOriginalNodesQuickActionButton = document.createElement("button");
    exportOriginalNodesQuickActionButton.type = "button";
    exportOriginalNodesQuickActionButton.className = "graph-quick-action-menu-item";
    exportOriginalNodesQuickActionButton.setAttribute("role", "menuitem");
    exportOriginalNodesQuickActionButton.innerHTML = `<i class="${CONTEXT_MENU_ACTIONS.exportOriginalNodes.icon}" aria-hidden="true"></i><span>${CONTEXT_MENU_ACTIONS.exportOriginalNodes.label}</span>`;
    quickActionMenu.appendChild(healthReportButton);
    ...
    quickActionMenu.appendChild(exportOriginalNodesQuickActionButton);
    quickActionWrapper.append(quickActionFailureStatus, quickActionStatus, quickActionMenu, quickActionButton);
    graphRenderWrapper.appendChild(quickActionWrapper);

    const setGraphQuickActionBusy = (busy, message = "", options = {}) => {
      const failedCount = Math.max(0, Math.floor(Number(options.failedCount) || 0));
      quickActionWrapper.classList.toggle("busy", !!busy);
      quickActionButton.disabled = !!busy;
      healthReportButton.disabled = !!busy;
      groupMostReferencedButton.disabled = !!busy;
      groupUngroupedButton.disabled = !!busy;
      removeLeafQuickActionButton.disabled = !!busy;
      exportOriginalNodesQuickActionButton.disabled = !!busy;
      ...
      quickActionFailureStatus.textContent = busy && failedCount > 0
        ? `Failed nodes ${failedCount.toLocaleString()}`
        : "";
      quickActionFailureStatus.classList.toggle("hidden", !busy || failedCount <= 0);
    ...
    healthReportButton.addEventListener("click", () => {
      closeGraphQuickActionMenu();
      openGraphHealthReportTab(activeTab);
    });
    ...
    exportOriginalNodesQuickActionButton.addEventListener("click", async () => {
      closeGraphQuickActionMenu();
    ...
    const applyMagneticSetting = () => {
      if (graphSettings.magneticEnabled) {
        ...
        if (useStaticLargeGraphLayout) {
          const staticTickStart = typeof performance !== "undefined" ? performance.now() : 0;
          simulation.alpha(0.85).tick(staticLargeGraphTickCount).stop();
          graphPerf?.mark("static large graph simulation ticks", {
            ticks: staticLargeGraphTickCount,
            durationMs: elapsedGraphPerfMs(staticTickStart),
            nodes: nodes.length,
            links: links.length
          });
          const tickRenderStart = typeof performance !== "undefined" ? performance.now() : 0;
          renderGraphTick();
          graphPerf?.mark("static large graph tick render", {
            durationMs: elapsedGraphPerfMs(tickRenderStart),
            nodes: nodes.length,
            links: links.length
          });
        } else {
        ...
        const tickRenderStart = typeof performance !== "undefined" ? performance.now() : 0;
        renderGraphTick();
        graphPerf?.mark("non-magnetic graph tick render", {
          durationMs: elapsedGraphPerfMs(tickRenderStart),
          nodes: nodes.length,
          links: links.length
        });
    ...
    node.on("contextmenu", (event, d) => {
      ...
      const isMissingDependency = isMissingDependencyNode(d);
      const isFileNode = !isTag && !isCluster && !isMissingDependency;
    ...
    openFileBtn.addEventListener("click", async (event) => {
      ...
      const targetNode = contextTargetNode;
      hideContextMenu();
      await openGraphNodeFileFromCurrentRender(targetNode);
    });
    ...
    revealTreeViewBtn.addEventListener("click", async (event) => {
      event.stopPropagation();
      if (!contextTargetNode) return;
      const targetNode = contextTargetNode;
      hideContextMenu();
      if (!await revealGraphNodeInTreeView(targetNode)) {
    ...
    addTagBtn.addEventListener("click", async (event) => {
      ...
      const tag = await promptForNewTag({
        title: "Add tag",
        message: "Enter a tag to add to this file.",
      ...
      try {
        ...
        await withPausedGraphDraftPersistence("graph:add-tag", () => updateGraphNodeTagContent(targetNode, normalizedTag, "add"));
      } catch (error) {
    ...
    removeTagBtn.addEventListener("click", async (event) => {
      ...
      try {
        ...
        const tag = await app.services.prompt({
          title: "Remove tag",
        ...
        await withPausedGraphDraftPersistence("graph:remove-tag", () => updateGraphNodeTagContent(targetNode, normalizedTag, "remove"));
      } catch (error) {
    ...
    deleteTagBtn.addEventListener("click", async (event) => {
      ...
      hideContextMenu();
      try {
        await withPausedGraphDraftPersistence("graph:delete-tag", () => deleteTag(tag));
      } catch (error) {
    ...
    deleteFileBtn.addEventListener("click", async (event) => {
      ...
      const confirmed = typeof shouldConfirmDeleteFiles === "function" && !shouldConfirmDeleteFiles()
        ...
        : (typeof app?.services?.confirm === "function"
          ? await app.services.confirm({
              message: `Delete "${getNodeFileName(nodeId)}" from disk? This action cannot be undone.`,
              confirmLabel: "Delete",
      ...
      try {
        ...
        simulation.stop();
        await removeGraphPointFromSnapshot(nodeId);
    ...
    const openLocalGraphTab = async (mode, titlePrefix) => {
      if (!contextTargetNode) return;
      const focusNodeId = contextTargetNode.id;
      if (hasReachedOpenTabLimit()) {
        return;
      }
      ...
      const nodeAnchorPath = getNodeFilesystemPath(contextTargetNode) || contextTargetNode.fullPath || contextTargetNode.path || "";
      const localGraphTab = await createGraphTab(localTabTitle, {
        ...
        openedSource: typeof createOpenedSource === "function"
          ? createOpenedSource(nodeAnchorPath, contextTargetNode.label || getFileName(nodeAnchorPath), "file-graph")
          : null
      });
    ...
    const openExpandedClusterGraphTab = async () => {
      ...
      const clusterGraphTab = await createGraphTab(`Expanded Cluster: ${contextTargetNode.label}`, {
        graphSnapshot: activeGraphTab?.graphSnapshot || null,
        graphViewConfig: {
          ...parentConfig,
          mode: "cluster",
        ...
        graphLayout: getGraphLayoutWithoutZoom(activeGraphTab?.graphLayout),
        openedSource: activeGraphTab?.openedSource || null
      });
    ...
    graphRenderWrapper.style.setProperty("--graph-link-color", typeof getGraphLinkColor === "function" ? getGraphLinkColor() : "#30363d");
    graphRenderWrapper.style.setProperty("--graph-external-dependency-color", typeof getGraphExternalDependencyColor === "function" ? getGraphExternalDependencyColor() : "#c26a1b");
    graphRenderWrapper.style.setProperty("--graph-external-dependency-line-color", typeof getGraphExternalDependencyLineColor === "function" ? getGraphExternalDependencyLineColor() : "#5e4e40");
    graphRenderWrapper.style.setProperty("--graph-missing-dependency-color", typeof getGraphMissingDependencyColor === "function" ? getGraphMissingDependencyColor() : "#1a288e");
    graphRenderWrapper.style.setProperty("--graph-missing-dependency-line-color", typeof getGraphMissingDependencyLineColor === "function" ? getGraphMissingDependencyLineColor() : "#6c76f9");
    graphRenderWrapper.style.setProperty("--graph-tag-node-color", typeof getGraphTagNodeColor === "function" ? getGraphTagNodeColor() : "#14b8a6");
    graphRenderWrapper.style.setProperty("--graph-tag-line-color", typeof getGraphTagLineColor === "function" ? getGraphTagLineColor() : "#6e7781");
    graphRenderWrapper.style.setProperty("--graph-cluster-node-color", typeof getGraphClusterNodeColor === "function" ? getGraphClusterNodeColor() : "#f59e0b");
    ...
    removeGraphLoadingState();
    graphPerf?.mark("loading state removed");
    applyMagneticSetting();
    graphPerf?.mark("magnetic layout applied", {
      staticLargeGraph: useStaticLargeGraphLayout,
      staticTickCount: staticLargeGraphTickCount,
    ...
    const api = { renderGraphView, openGraphFindDialog, canReuseGraphRender, activateCachedGraphRender };
    app.services.graphRenderer = api;
    app.registerModule?.("graphRenderer", api);
    return api;
  };
})(window);
