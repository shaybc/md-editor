(function(global) {
  ...
  function extractSourceFileFromFrontmatter(markdown) {
    const frontmatterMatch = getMarkdownFrontmatterMatch(markdown);
    if (!frontmatterMatch) return "";

    if (typeof jsyaml !== "undefined" && jsyaml?.load) {
      try {
        const data = jsyaml.load(frontmatterMatch[1]) || {};
        if (data && Object.prototype.hasOwnProperty.call(data, "source_file")) {
          return normalizeFrontmatterScalar(data.source_file);
        }
      } catch (error) {
        console.warn("Frontmatter source_file YAML parse error:", error);
      }
    }

    const inlineSourceFileMatch = frontmatterMatch[1].match(/^[ \t]*source_file[ \t]*:[ \t]*(.+?)[ \t]*$/im);
    return inlineSourceFileMatch ? normalizeFrontmatterScalar(inlineSourceFileMatch[1]) : "";
  }
...
${body}`;
  ...
  function extractUnresolvedDependencies(markdown) {
    const source = String(markdown || "");
    const frontmatterMatch = getMarkdownFrontmatterMatch(source);
    const sourceLanguageMatch = frontmatterMatch
      ? frontmatterMatch[1].match(/^\s*language\s*:\s*([^\r\n#]+)/im)
      : null;
    const sourceLanguage = sourceLanguageMatch ? normalizeFrontmatterScalar(sourceLanguageMatch[1]).toLowerCase() : "";
    const sectionMatch = source.match(/^## Unresolved Dependencies\s*$(.*?)(?=^##\s+|$(?![\s\S]))/ims);
    ...
    while ((match = lineRegex.exec(sectionMatch[1])) !== null) {
      ...
      if (!symbol) continue;
      const detail = String(match[2] || "").toLowerCase();
      const lineMatch = detail.match(/\bline\s+(\d+)\b/);
      const languageMatch = detail.match(/\blanguage\s+([a-z0-9_-]+)\b/);
      const line = lineMatch ? Number(lineMatch[1]) : null;
      dependencies.push({
        symbol,
        kind: detail.includes("package") ? "package" : detail.includes("static") ? "static-owner" : "class",
        wildcard: detail.includes("wildcard"),
        staticImport: detail.includes("static"),
        language: languageMatch?.[1] || sourceLanguage || "java",
    ...
    return dependencies;
      ...
      api.extractSourceFileFromFrontmatter = extractSourceFileFromFrontmatter;
  ...
  api.extractUnresolvedDependencies = extractUnresolvedDependencies;
    }
