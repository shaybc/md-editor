(function(global) {
  ...
  function setGraphViewMode(enabled) {
    ...
    const activeGraphViewRoot = typeof getActiveGraphViewRoot === "function" ? getActiveGraphViewRoot() : null;
    const activePreviewPane = typeof getActivePreviewPane === "function" ? getActivePreviewPane() : null;
    const previewPane = activeGraphViewRoot || activePreviewPane || document.querySelector(".tab-view.active .preview-pane, .preview-pane");
  ...
  function updateGraphTagToolbar(tab, graphSnapshot) {
    ...
    const graphControlInputs = [
      graphDisplayArrows,
      graphDisplayExternalJars,
      graphDisplayMissingDependencies,
      graphDisplayOrphans,
      graphDisplayLabels,
    ...
    if (graphDisplayExternalJars) graphDisplayExternalJars.checked = graphViewConfig.showExternalJars;
    if (graphDisplayMissingDependencies) graphDisplayMissingDependencies.checked = graphViewConfig.showMissingDependencies;
  ...
  function resetActiveGraphViewToDefaults() {
    ...
    updateActiveGraphViewConfig({
      ...
      showExternalJars: DEFAULT_GRAPH_VIEW_CONFIG.showExternalJars,
  ...
  function updateActiveGraphViewConfig(patch, options = {}) {
    ...
    const perf = typeof createGraphPerfSession === "function"
      ? createGraphPerfSession("graph view config update", {
        tabId: activeGraphTab.id || "",
        title: activeGraphTab.title || activeGraphTab.folderName || "",
        patchKeys,
        skipToolbar: Boolean(options.skipToolbar),
        persistGraphPreferences: Boolean(options.persistGraphPreferences)
      })
      : null;
    const nowForConfigPerf = () => (typeof performance !== "undefined" ? performance.now() : 0);
    const elapsedConfigPerfMs = (startTime) => {
      if (!startTime || typeof performance === "undefined") return 0;
      return Math.round((performance.now() - startTime) * 10) / 10;
    };
    let stepStart = nowForConfigPerf();
    activeGraphTab.graphViewConfig = normalizeGraphViewConfig({
      ...(activeGraphTab.graphViewConfig || {}),
      ...patch
    });
    perf?.mark("normalize config", {
      durationMs: elapsedConfigPerfMs(stepStart),
      groups: activeGraphTab.graphViewConfig.groups?.length || 0,
      hiddenGroups: (activeGraphTab.graphViewConfig.groups || []).filter((group) => group?.hidden === true).length,
      showExternalJars: activeGraphTab.graphViewConfig.showExternalJars
    });
    stepStart = nowForConfigPerf();
    ...
    perf?.mark("invalidate render and toolbar", {
      durationMs: elapsedConfigPerfMs(stepStart),
      skipToolbar: Boolean(options.skipToolbar)
    });
    stepStart = nowForConfigPerf();
    markGraphTabAsChanged(activeGraphTab);
    perf?.mark("mark graph changed", {
      durationMs: elapsedConfigPerfMs(stepStart)
    });
    stepStart = nowForConfigPerf();
    ...
    perf?.mark("persist graph preferences", {
      durationMs: elapsedConfigPerfMs(stepStart),
      enabled: Boolean(options.persistGraphPreferences)
    });
    stepStart = nowForConfigPerf();
    const renderResult = renderGraphView({ skipToolbar: options.skipToolbar });
    if (renderResult && typeof renderResult.then === "function") {
      renderResult
        .then(() => {
          perf?.mark("render graph view complete", {
            durationMs: elapsedConfigPerfMs(stepStart)
          });
          perf?.end({ completed: true });
        })
        .catch((error) => {
          perf?.mark("render graph view failed", {
            durationMs: elapsedConfigPerfMs(stepStart),
            message: error?.message || String(error)
          });
          perf?.end({ completed: false });
        });
    } else {
      perf?.mark("render graph view requested", {
        durationMs: elapsedConfigPerfMs(stepStart)
      });
      perf?.end({ completed: true });
    }
  ...
  desktopOpenGraphButtons.forEach((button) => button.addEventListener("click", (event) => {
    if (event.currentTarget?.disabled) return;
    openGraphView();
  }));
  if (mobileOpenGraphView) mobileOpenGraphView.addEventListener("click", (event) => {
    if (event.currentTarget?.disabled) return;
    openGraphView();
  });
  ...
  if (graphDisplayExternalJars) graphDisplayExternalJars.addEventListener("change", () => updateActiveGraphViewConfig({ showExternalJars: graphDisplayExternalJars.checked }));
  if (graphDisplayMissingDependencies) graphDisplayMissingDependencies.addEventListener("change", () => updateActiveGraphViewConfig({ showMissingDependencies: graphDisplayMissingDependencies.checked }, { persistGraphPreferences: true }));
  ...
  function initializeGraphFilterTooltips() {
    ...
    const tooltipTextBySelector = [
      ...
      ["#graph-display-external-jars", "Toggle external dependency points generated by code conversion."],
      ["#graph-display-missing-dependencies", "Toggle unresolved dependency points generated by code conversion."],
