...
(function(window, document) {
  ...
  function createExtensionEditor(deps) {
    ...
    function open(options) {
      const empty = { manifest: { schemaVersion: 2, id: "", name: "", version: "1.0.0", description: "" }, skills: [], agents: [], hooks: [], mcpServers: [], tools: [], commands: [] };
    ...
    function collect() {
      state.draft.manifest = { schemaVersion: 2, id: fields.id.value.trim(), name: fields.name.value.trim(), version: fields.version.value.trim(), description: fields.description.value.trim() };
  ...
  function buildContributionDialog(group, existing) {
    const kind = { skills: "Skill", agents: "Agent", hooks: "Hook", mcpServers: "External server", tools: "Tool", commands: "Command" }[group];
    ...
    overlay.innerHTML = `<div class="reset-modal-box settings-ai-extension-contribution-box" role="dialog" aria-modal="true"><header class="settings-ai-extension-editor-header"><button class="settings-modal-close" data-close type="button"><span aria-hidden="true">&times;</span></button><p class="reset-modal-message">${existing ? "Edit" : "Add"} ${kind.toLowerCase()}</p></header><div class="settings-ai-extension-editor-body">${common ? `${field("id", `${kind} ID`, "text", "id")}${field("name", "Name", "text", "name")}${textareaField("description", "Description", 3, "description")}${textareaField("body", "Markdown instructions", 12, "body")}${textareaField("metadata", "Additional metadata (JSON object)", 8, "metadata")}` : ""}${hook ? `${field("id", "Hook ID", "text", "id")}${field("event", "Event", "text", "event")}${selectField("enabled", "Status", [{ value: "true", label: "Enabled" }, { value: "false", label: "Disabled" }], "event")}${selectField("onError", "When an action fails", [{ value: "continue", label: "Continue" }, { value: "block", label: "Block current boundary" }, { value: "stop-run", label: "Stop run" }], "event")}<div class="settings-field-grid settings-field-grid-two">${field("timeoutMs", "Timeout (ms)", "number", "event")}${field("cooldownMs", "Cooldown (ms)", "number", "event")}${field("dedupWindowMs", "Deduplication window (ms)", "number", "event")}${selectField("background", "Execution", [{ value: "false", label: "Wait for completion" }, { value: "true", label: "Run in background" }], "event")}</div>${textareaField("matcher", "Matcher (JSON object)", 5, "event")}${textareaField("actions", "Ordered actions (JSON array)", 10, "actions")}` : ""}${server ? `${field("id", "Server ID", "text", "id")}${selectField("transport", "Transport", [{ value: "http", label: "HTTP" }, { value: "stdio", label: "Local process" }], "server")}${field("url", "Server URL", "url", "server")}${field("command", "Executable command", "text", "server")}${textareaField("args", "Arguments (JSON array)", 4, "server")}${textareaField("headers", "Headers (JSON object)", 4, "server")}${textareaField("env", "Environment (JSON object)", 4, "server")}${field("cwd", "Working directory", "text", "server")}` : ""}${tool ? `${field("id", "Tool name", "text", "id")}${textareaField("description", "Description", 3, "description")}${selectField("adapterType", "Execution adapter", [{value:"application-action",label:"MD-Editor action"},{value:"command",label:"Approved command"},{value:"web-request",label:"Approved web request"},{value:"external-tool",label:"External server tool"},{value:"workflow",label:"Workflow skill"}], "server")}${field("target", "Adapter target", "text", "server")}${field("executable", "Command executable", "text", "server")}${textareaField("adapterArgs", "Command arguments (JSON array)", 4, "server")}${field("url", "Web URL", "url", "server")}${textareaField("headers", "Web headers (JSON object)", 4, "server")}${textareaField("inputSchema", "Input schema (JSON object)", 9, "metadata")}${field("allowedModes", "Allowed modes (comma-separated)", "text", "metadata")}${field("requiredCapability", "Required capability", "text", "metadata")}${field("permissionScope", "Permission scope", "text", "metadata")}${selectField("alwaysLoad", "Schema loading", [{value:"false",label:"Deferred"},{value:"true",label:"Always loaded"}], "metadata")}` : ""}${command ? `${field("id", "Command ID", "text", "id")}${field("name", "Display name", "text", "name")}${textareaField("description", "Description", 3, "description")}${selectField("commandType", "Command type", [{value:"prompt",label:"Prompt expansion"},{value:"workflow",label:"Workflow skill"},{value:"tool",label:"Tool invocation"},{value:"application-action",label:"MD-Editor action"}], "metadata")}${field("target", "Target", "text", "metadata")}${field("aliases", "Aliases (comma-separated)", "text", "metadata")}${field("argumentHint", "Argument hint", "text", "metadata")}${field("allowedModes", "Allowed modes (comma-separated)", "text", "metadata")}${textareaField("body", "Markdown expansion or instructions", 12, "body")}` : ""}<p class="settings-panel-description" data-status aria-live="polite"></p></div><footer class="reset-modal-actions settings-ai-extension-editor-footer"><button class="reset-modal-btn reset-modal-cancel" data-cancel type="button">Cancel</button><button class="reset-modal-btn settings-primary-action" data-save type="button">Save contribution</button></footer></div>`;
    if (command) overlay.querySelector('[data-extension-field="body"]').closest("label").insertAdjacentHTML("beforebegin", selectField("modelInvocable", "Model access", [{ value: "false", label: "User slash command only" }, { value: "true", label: "Also expose as a deferred tool" }], "metadata"));
    ...
    if (tool) { const definition = value.definition || {}; inputs.id.value = definition.name || definition.id || ""; inputs.description.value = definition.description || ""; inputs.adapterType.value = definition.adapter?.type || "workflow"; inputs.target.value = definition.adapter?.target || ""; inputs.executable.value = definition.adapter?.executable || ""; inputs.adapterArgs.value = JSON.stringify(definition.adapter?.args || [], null, 2); inputs.url.value = definition.adapter?.url || ""; inputs.headers.value = JSON.stringify(definition.adapter?.headers || {}, null, 2); inputs.inputSchema.value = JSON.stringify(definition.inputSchema || {type:"object",properties:{},additionalProperties:false}, null, 2); inputs.allowedModes.value = (definition.allowedModes || ["agent"]).join(", "); inputs.requiredCapability.value = definition.requiredCapability || ""; inputs.permissionScope.value = definition.permissionScope || ""; inputs.alwaysLoad.value = String(definition.alwaysLoad === true); }
    if (command) { const metadata = value.metadata || {}; inputs.id.value = metadata.id || ""; inputs.name.value = metadata.name || ""; inputs.description.value = metadata.description || ""; inputs.commandType.value = metadata.type || "prompt"; inputs.target.value = metadata.target || ""; inputs.aliases.value = (metadata.aliases || []).join(", "); inputs.argumentHint.value = metadata.argumentHint || ""; inputs.allowedModes.value = (metadata.allowedModes || ["agent"]).join(", "); inputs.modelInvocable.value = String(metadata.modelInvocable === true); inputs.body.value = value.body || ""; }
    return { overlay, close: overlay.querySelector("[data-close]"), cancel: overlay.querySelector("[data-cancel]"), save: overlay.querySelector("[data-save]"), status: overlay.querySelector("[data-status]"), read() { if (common) return { metadata: { ...parseObject(inputs.metadata.value), id: inputs.id.value.trim(), name: inputs.name.value.trim(), description: inputs.description.value.trim() }, body: inputs.body.value }; if (hook) return { definition: { id: inputs.id.value.trim(), event: inputs.event.value.trim(), enabled: inputs.enabled.value === "true", onError: inputs.onError.value, timeoutMs: Number(inputs.timeoutMs.value), cooldownMs: Number(inputs.cooldownMs.value), dedupWindowMs: Number(inputs.dedupWindowMs.value), background: inputs.background.value === "true", matcher: parseObject(inputs.matcher.value), actions: parseArray(inputs.actions.value) } }; if (tool) { const type = inputs.adapterType.value; const adapter = type === "command" ? {type, executable:inputs.executable.value.trim(), args:parseArray(inputs.adapterArgs.value)} : (type === "web-request" ? {type,url:inputs.url.value.trim(),headers:parseObject(inputs.headers.value)} : {type,target:inputs.target.value.trim()}); return { definition: { id:inputs.id.value.trim(), name:inputs.id.value.trim(), description:inputs.description.value.trim(), adapter, inputSchema:parseObject(inputs.inputSchema.value), allowedModes:splitList(inputs.allowedModes.value), requiredCapability:inputs.requiredCapability.value.trim(), permissionScope:inputs.permissionScope.value.trim(), alwaysLoad:inputs.alwaysLoad.value === "true" } }; } if (command) return { metadata:{id:inputs.id.value.trim(),name:inputs.name.value.trim(),description:inputs.description.value.trim(),type:inputs.commandType.value,target:inputs.target.value.trim(),aliases:splitList(inputs.aliases.value),argumentHint:inputs.argumentHint.value.trim(),allowedModes:splitList(inputs.allowedModes.value),modelInvocable:inputs.modelInvocable.value === "true"},body:inputs.body.value }; return { definition: { id: inputs.id.value.trim(), transport: inputs.transport.value, url: inputs.url.value.trim(), command: inputs.command.value.trim(), args: parseArray(inputs.args.value), headers: parseObject(inputs.headers.value), env: parseObject(inputs.env.value), cwd: inputs.cwd.value.trim() } }; } };
  }
  ...
  function splitList(text) { return String(text || "").split(",").map((value) => value.trim()).filter(Boolean); }
