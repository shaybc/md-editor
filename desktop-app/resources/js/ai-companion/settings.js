  ...
  function registerMarkdownViewerAiCompanionSettings(app) {
    ...
    function normalize(settings) {
      ...
      return {
        ...
        connectionProfiles: Array.isArray(source.connectionProfiles) ? source.connectionProfiles.map(function(entry) { const normalized = Object.assign({}, entry, { id: String(entry && entry.id || "").trim(), providerMode: String(entry && entry.providerMode || "openai-compatible"), model: String(entry && entry.model || "").trim() }); delete normalized.apiKey; delete normalized.geminiConnectorApiKey; return normalized; }).filter(function(entry) { return entry.id; }).slice(0, 30) : [],
