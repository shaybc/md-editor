(function(global) {
  ...
  function clearFolderMarkdownContentCache() {
    folderMarkdownContentCache.clear();
  }
  ...
  async function promptForNewTag(options = {}) {
    const notify = app?.services?.notify;
    if (!notify?.show) return "";

    let tagInput = null;
    const result = await notify.show({
      title: options.title || "Create tag",
      message: options.message || "Enter a tag name.",
      dismissValue: null,
      buttons: [
        { id: "cancel", label: options.cancelLabel || "Cancel", value: null, variant: "cancel" },
        {
          id: "create",
          label: options.confirmLabel || "Create",
          variant: "primary",
          autoFocus: true,
          action: () => normalizeTagName(tagInput?.value || "") || null
        }
      ],
      renderBody(body) {
        tagInput = document.createElement("input");
        tagInput.className = "rename-modal-input";
        tagInput.type = "text";
        tagInput.value = String(options.defaultValue || "");
        tagInput.setAttribute("aria-label", options.inputLabel || "Tag name");
    ...
    return typeof result === "string" ? normalizeTagName(result) : "";
  }
  ...
  async function deleteTag(tagName, options = {}) {
    ...
    const confirmed = options.skipConfirmation === true
      || (typeof app?.services?.confirm === "function"
        ? await app.services.confirm({
            message: `Delete tag "#${normalizedTag}"? This removes it from every file that has the tag and saves those files.`,
            confirmLabel: "Delete",
            confirmVariant: "danger"
          })
        : window.confirm(`Delete tag "#${normalizedTag}"? This removes it from every file that has the tag and saves those files.`));
