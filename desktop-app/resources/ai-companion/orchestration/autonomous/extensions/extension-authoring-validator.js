...
const GROUPS = Object.freeze({ skills: "skill", agents: "agent", hooks: "hook", mcpServers: "mcp-server", tools: "tool", commands: "command" });

function validateAuthoredExtension(draft) {
  const errors = [];
  const warnings = [];
  let manifest;
  try { manifest = normalizeExtensionManifest({ ...(draft?.manifest || {}), schemaVersion: 2, contributions: { skills: [], agents: [], hooks: [], mcpServers: [], tools: [], commands: [] } }); }
  catch (error) { errors.push(error.message); }
  const normalized = { manifest, skills: [], agents: [], hooks: [], mcpServers: [], tools: [], commands: [] };
...
function normalizeContribution(kind, entry) {
  if (kind === "skill" || kind === "agent" || kind === "command") {
    ...
    } else Object.assign(metadata, normalizeExtensionCommand(metadata));
  ...
  const normalized = kind === "hook" ? normalizeHookDefinition(definition) : (kind === "tool" ? normalizeExtensionTool(definition) : normalizeServerConfiguration(definition));
...
function safeContributionPath(candidate, kind, id, extension) {
  const folder = kind === "mcp-server" ? "external-servers" : (kind === "command" ? "commands" : `${kind}s`);
