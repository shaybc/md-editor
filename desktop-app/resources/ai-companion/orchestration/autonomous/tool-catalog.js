...
const DEFINITIONS = Object.freeze([
  ...
  tool("work_create", "Create a model-controlled work item when a complex task has several independent steps, a long tool sequence, or delegated work. Skip this for simple requests.", { subject: text("Brief work title"), description: text("Required outcome"), activeForm: text("Present-continuous progress label"), metadata: { type: "object" } }, ["subject", "description"]),
  tool("work_get", "Read one work item.", { id: text("Work item id") }, ["id"]),
  tool("work_list", "List work items in stable order.", {}),
  tool("work_update", "Update, complete, assign, link, or delete a work item. Mark current work in_progress, complete it only after verification, and delete obsolete items.", { id: text("Work item id"), subject: text("New title"), description: text("New description"), activeForm: text("Progress label"), owner: text("Worker id"), status: text("pending, in_progress, completed, or deleted"), addBlocks: { type: "array", items: text("Work item id") }, addBlockedBy: { type: "array", items: text("Work item id") }, metadata: { type: "object" } }, ["id"]),
