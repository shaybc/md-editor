...
/** Builds and installs the Java Push Down companion bundle used by JDT LS. */
function main() {
  const appRoot = __dirname;
  const projectRoot = path.join(appRoot, "language-server-extensions", "java-push-down");
  ...
  if (result.status !== 0) throw new Error("Java Push Down JDT extension build failed.");
  const source = path.join(projectRoot, "target", "mdeditor-java-push-down-jdt-1.0.0.jar");
  const destinationRoot = path.join(appRoot, "resources", "language-server-extensions");
  fs.mkdirSync(destinationRoot, { recursive: true });
  fs.copyFileSync(source, path.join(destinationRoot, "mdeditor-java-push-down.jar"));
}
